...
class TestSupplierManager:
    ...
    def test_get_supplier_by_name(self, manager, mock_db):
        ...
        row = (
            1, "Target Supplier", "Contact", "123", "email@test.com", "Addr", "TAX123", 1,
            datetime.now().isoformat(), datetime.now().isoformat(), "456", 1000.0, 50.0,
            "2023-01-01", 500.0, 5
        )
    ...
    def test_reporting_methods(self, manager, mock_db):
        ...
        mock_db.fetch_all.return_value = [
            ("INV-001", "2023-01-01", 100.0, 50.0, 50.0, "paid")
        ]
        ...
        row = (
            1, "Debtor", "Contact", "123", "email", "Addr", "Tax", 1,
            datetime.now().isoformat(), datetime.now().isoformat(), "456", 1000.0, 500.0, # Balance > 0
            None, 0, 0
        )
    ...
    def test_exceptions(self, manager, mock_db):
        ...
        with patch.object(manager, 'get_supplier_purchases_count', return_value=0), \
             patch.object(manager, 'get_supplier_products_count', return_value=0):
    ...
    def test_delete_supplier(self, manager, mock_db):
        ...
        with patch.object(manager, 'get_supplier_purchases_count', return_value=0), \
             patch.object(manager, 'get_supplier_products_count', return_value=5):
                assert manager.delete_supplier(1) is False
