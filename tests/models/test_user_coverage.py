...
class TestUserCoverage:
    ...
    def test_row_to_user_conversion(self, user_manager):
        ...
        row = (
            1, "user1", "email@test.com", "Full Name", "123456", "admin", "hash", "salt",
            1, 0, 0, datetime.now().isoformat(), datetime.now().isoformat(), 
            datetime.now().isoformat(), "notes", 1, datetime.now().isoformat(), datetime.now().isoformat()
        )
    ...
    def test_row_to_user_none_timestamps(self, user_manager):
        ...
        row = (
            2, "user2", None, "Name 2", None, "user", "hash", "salt",
    ...
    def test_authenticate_user_wrong_password_lockout(self, user_manager):
        ...
        with patch.object(user_manager, 'get_user_by_username', return_value=user), \
             patch.object(user_manager, '_verify_password', return_value=False):
    ...
    def test_change_password_wrong_old(self, user_manager):
        ...
        with patch.object(user_manager, 'get_user_by_id', return_value=user), \
             patch.object(user_manager, '_verify_password', return_value=False):
    ...
    def test_change_password_success(self, user_manager, mock_db):
        user = User(id=1)
        with patch.object(user_manager, 'get_user_by_id', return_value=user), \
             patch.object(user_manager, '_verify_password', return_value=True):
