...
class TestSaleManager:
    ...
    def test_create_sale_success(self, mock_db):
        ...
        mock_db.connection.execute.return_value.fetchall.return_value = [
            (0, 'id'), (1, 'invoice_number'), (2, 'total_amount'), 
            (3, 'status'), (4, 'created_at')
        ]
        ...
        with patch.object(manager, '_create_sale_item', return_value=1) as mock_create_item, \
             patch.object(manager, '_update_stock_for_sale') as mock_update_stock, \
             patch('src.services.webhook_service.WebhookService') as MockWebhook:
