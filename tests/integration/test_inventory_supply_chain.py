...
from src.services.advanced_inventory_management_service import AdvancedInventoryManagementService, InventoryAlert, InventoryOptimization
from src.services.supply_chain_integration_service import SupplyChainIntegrationService, PurchaseOrder, SupplierPerformance, SupplyChainAlert
...
class TestAdvancedInventoryManagementService(unittest.TestCase):
    ...
    def test_remove_inventory_item(self, mock_insert_transaction, mock_update_stock, mock_update_batch, mock_get_quantity):
    ...
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._check_damaged_stock_alerts')
    def test_get_inventory_alerts(self, mock_damaged, mock_overstock, mock_expiry, mock_low_stock):
        ...
        mock_low_stock.return_value = [
            InventoryAlert("LOW001", 1, 'low_stock', 'high', 'مخزون منخفض', 'إعادة طلب', datetime.now())
        ]
    ...
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._get_products_for_optimization')
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._calculate_inventory_optimization')
    def test_optimize_inventory(self, mock_calculate, mock_get_products):
        """اختبار تحسين المخزون"""
        mock_get_products.return_value = [
            {'id': 1, 'name': 'Product A', 'current_stock': 50, 'min_stock': 100, 'max_stock': 200, 'selling_price': 25.0}
        ]
    ...
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._generate_inventory_summary')
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._generate_product_inventory_report')
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._generate_warehouse_inventory_report')
    @patch('services.advanced_inventory_management_service.AdvancedInventoryManagementService._generate_transaction_report')
...
class TestSupplyChainIntegrationService(unittest.TestCase):
    ...
    def test_receive_purchase_order(self, mock_update_performance, mock_update_item, mock_get_cost,
                                  mock_update_delivery, mock_update_status, mock_get_po):
        ...
        received_items = [
            {'product_id': 1, 'batch_id': 'BATCH001', 'quantity': 100, 'warehouse_id': 1}
        ]
    ...
    def test_get_supply_chain_alerts(self, mock_risk, mock_delivery, mock_supplier):
        ...
        mock_supplier.return_value = [
            SupplyChainAlert("SUP001", 'supplier_issue', 'high', 'مشكلة مورد', [], ['إجراء'], datetime.now())
        ]
    ...
    def test_optimize_supplier_selection(self, mock_performance, mock_suppliers):
        ...
        mock_suppliers.return_value = [
            {'supplier_id': 1, 'name': 'Supplier A', 'unit_price': 10.5, 'lead_time_days': 5, 'status': 'active'}
        ]
    ...
    def test_generate_purchase_plan(self, mock_optimize, mock_reorder):
        ...
        mock_reorder.return_value = [
            {'product_id': 1, 'product_name': 'Product A', 'recommended_quantity': 100, 'priority': 'high'}
        ]

        mock_optimize.return_value = [
            {'supplier_name': 'Supplier A', 'total_cost': 1050.0, 'score': 0.9}
        ]
...
class TestIntegratedInventorySupplyChain(unittest.TestCase):
    ...
    def test_unified_alerts_system(self, mock_supply_alerts, mock_inventory_alerts):
        ...
        mock_inventory_alerts.return_value = [
            InventoryAlert("INV001", 1, 'low_stock', 'high', 'مخزون منخفض', 'إعادة طلب', datetime.now())
        ]

        mock_supply_alerts.return_value = [
            SupplyChainAlert("SUP001", 'supplier_issue', 'medium', 'مشكلة مورد', [], ['إجراء'], datetime.now())
        ]
