...
_MOCK_PAYLOAD = {"sub": "1", "username": "admin", "company_id": 1, "role": "admin"}


@pytest.fixture(autouse=True)
def mock_auth():
    """تعطيل التحقق من JWT وDB lookup للمصادقة"""
    with patch.object(auth_manager, "verify_token", return_value=_MOCK_PAYLOAD), patch.object(
        auth_manager, "get_current_user", return_value=_MOCK_USER
    ):
        yield
...
class TestSyncAPI:
    ...
    def test_handshake(self):
        with TestClient(app) as client:
            response = client.get("/api/v1/sync/handshake", headers={"Authorization": "Bearer test"})
    ...
    def test_push_sync(self):
        ...
        payload = {"table_name": "products", "items": [{"name": "New Product"}]}
