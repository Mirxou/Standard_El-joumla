...
from src.services.ui_adaptation_service import UIAdaptationService, UIElement
...
class TestUnifiedCommerceServices(unittest.TestCase):
    ...
    def test_cpq_service_quote_creation(self):
        ...
        with patch.object(CPQService, '_get_product') as mock_get_product, \
             patch.object(CPQService, '_get_product_options') as mock_get_options, \
             patch.object(CPQService, '_save_quote') as mock_save:
    ...
    def test_cpq_configuration_validation(self):
        ...
        # Valid configuration
        valid_config = ProductConfiguration(
            product_id=1,
        ...
        invalid_config = ProductConfiguration(
            product_id=1,
            selected_options={'color': 'blue'},
            quantity=-1
        )
