...
def test_database_tables(db_manager: DatabaseManager) -> bool:
    ...
    required_tables = ["warehouses", "warehouse_inventory", "warehouse_transfers"]
...
def test_inventory_migration(db_manager: DatabaseManager, warehouse_service: WarehouseService) -> bool:
    ...
    try:
        ...
        for product in products:
            ...
            inventory = warehouse_service.inventory_manager.get_inventory(default_warehouse.id, product_id)
            ...
            if inventory:
                if inventory.quantity == old_stock:
                    ...
                    print_warning(
                        f"المنتج '{product_name}': المخزون غير متطابق (قديم: {old_stock}, جديد: {inventory.quantity})"
                    )
            else:
                if old_stock == 0:
...
def test_warehouse_transfer(db_manager: DatabaseManager, warehouse_service: WarehouseService) -> bool:
    ...
    try:
        ...
        test_warehouse = Warehouse(code=test_code, name="مستودع نقل اختبار", is_active=True, is_default=False)
        ...
        if transfer_id:
            ...
            if warehouse_service.complete_transfer(transfer_id, received_by=1):
                ...
                # التحقق من المخزون
                source_inv = warehouse_service.inventory_manager.get_inventory(default_warehouse.id, product_id)
                dest_inv = warehouse_service.inventory_manager.get_inventory(test_wh_id, product_id)
...
def main():
    ...
    results.append(
        (
            ...
            test_inventory_service_backward_compatibility(db_manager),
        )
    )
    results.append(
        (
            "نقل المخزون بين المستودعات",
            test_warehouse_transfer(db_manager, warehouse_service),
        )
    )
