...
def mock_services():
    ...
    payment_service.get_accounts_receivable.return_value = [{"balance": 5000.0}]
    payment_service.get_accounts_payable.return_value = [{"balance": 3000.0}]
...
def main_window(qapp, db_manager, mock_services):
    ...
    with patch("src.core.config_manager.ConfigManager") as mock_config, patch(
        "src.services.dashboard_service.DashboardService",
        return_value=mock_services["dashboard"],
    ), patch(
        "src.services.purchase_service.PurchaseService",
        return_value=mock_services["purchase"],
    ), patch(
        "src.services.payment_service.PaymentService",
        return_value=mock_services["payment"],
    ), patch(
        "src.services.report_exporter.ReportExporter",
        return_value=mock_services["exporter"],
    ), patch(
        "src.services.hardware_service.HardwareService"
    ), patch(
        "src.services.system_doctor_service.SystemDoctorService"
    ), patch(
        "src.services.fiscal_service.FiscalService"
    ), patch(
        "src.services.ai_prediction_service.AIPredictionService"
    ), patch(
        "src.services.workflow_service.WorkflowService"
    ), patch(
        "src.services.smart_assistant.SmartAssistantService"
    ), patch(
        "src.ui.widgets.smart_assistant_widget.SmartAssistantWidget"
    ), patch(
        "src.services.carbon_service.CarbonService"
    ), patch(
        "src.services.sentiment_service.SentimentService"
    ), patch(
        "src.services.gamification_service.GamificationService"
    ), patch(
        "src.services.approval_service.ApprovalService"
    ), patch(
        "src.services.qrcode_service.QRCodeService"
    ):
