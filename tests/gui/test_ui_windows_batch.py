...
ALL_UI_COMPONENTS = get_ui_classes(WINDOWS_DIR, "src.ui.windows") + get_ui_classes(DIALOGS_DIR, "src.ui.dialogs")
...
def test_ui_component_instantiation(qtbot, mock_db, mock_inv, ui_class, module_name):
    ...
    with patch("src.ui.theme_manager.get_theme_manager"), patch(
        "src.ui.notifications_manager.get_notifications_manager"
    ), patch("src.core.config_manager.ConfigManager"), patch(
        "src.ui.widgets.custom_title_bar.CustomTitleBar", return_value=MagicMock()
    ), patch(
        "src.ui.animations.animation_manager.AnimationManager", return_value=MagicMock()
    ), patch(
