...
from src.models.payment import Payment, PaymentType, PaymentStatus, PaymentMethod, AccountType
...
class TestPaymentServiceCoverage:
    ...
    def service(self, db_manager):
        ...
        with patch('src.services.payment_service.PaymentManager'), \
             patch('src.services.payment_service.CustomerManager'), \
             patch('src.services.payment_service.SupplierManager'), \
             patch('src.services.payment_service.ExchangeRateService'):
    ...
    def test_create_supplier_payment_success(self, service):
        ...
        service.payment_manager.get_payment_by_id.return_value = Mock(
            id=1, company_id=1, to_dict=lambda: {"id": 1}
        )
    ...
    def test_get_overdue_receivables(self, service):
        ...
        p1 = Mock(customer_id=1, payment_type=PaymentType.CUSTOMER_PAYMENT.value, due_date=today - timedelta(days=10), amount=100)
        p2 = Mock(customer_id=2, payment_type=PaymentType.CUSTOMER_PAYMENT.value, due_date=today - timedelta(days=50), amount=200) # More overdue
    ...
    def test_create_supplier_payment_webhook_silent_fail(self, service):
        ...
        with patch('src.services.webhook_service.WebhookService', side_effect=Exception("Import Error")):
