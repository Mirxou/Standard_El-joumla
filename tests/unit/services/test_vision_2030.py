...
class MockDBManager:
    def __init__(self):
        self.data = {
            ...
            "purchases": [],
            "purchase_items": [],
            "suppliers": [{"id": 1, "name": "Test Supplier", "is_active": 1}],
        }
        self.last_id = 100
    ...
    def fetch_one(self, query, params=()):
        ...
        if "SELECT * FROM products WHERE id" in query:
            ...
            p = self.data["products"][0]  # noqa: F841
...
def test_smart_pricing():
    ...
    # Mock services
    db = MockDBManager()
    sales_service = SalesService(db)  # noqa: F841
    ...
    price_retail = (
        p.wholesale_price
        if (hasattr(p, "min_wholesale_qty") and qty_retail >= p.min_wholesale_qty)
    ...
    price_wholesale = (
        p.wholesale_price
        if (hasattr(p, "min_wholesale_qty") and qty_wholesale >= p.min_wholesale_qty)
        else p.selling_price
    )
