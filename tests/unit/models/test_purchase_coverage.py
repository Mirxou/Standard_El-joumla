...
class TestPurchaseModel:
    def test_purchase_initialization_and_calculations(self):
        purchase = Purchase(supplier_id=1, shipping_cost=50)
        ...
        expected_subtotal = item1.total_amount + item2.total_amount  # (assuming 15% tax default)  # noqa: F841
        # item1: 1000 + 150 = 1150
        # item2: 1000 + 150 = 1150
        # Total items: 2300
...
class TestPurchaseManager:
    ...
    def test_create_purchase_success(self, manager, mock_db, mock_signals, mock_webhook_service):
        ...
        mock_webhook_service.return_value.trigger_webhook.assert_called_with(
            event_type="purchase_created", payload=ANY, entity_id=123, company_id=None
        )
    ...
    def test_get_purchase_by_id(self, manager, mock_db):
        ...
        mock_db.fetch_one.return_value = (
            ...
            1.0,
            None,
            None,  # Multi-currency (idx 17-20)
            None,
            None,
            None,
            None,  # Notes, user, dates (idx 21-24)
            "Supplier A",  # Supplier name (idx 25)
        )

        # Mock items rows
        # Schema: id, purchase_id, product_id, q_ord, q_rec, cost, disc_p, disc_a, tax_p, tax_a, total, expiry, batch, notes, p_name, p_barcode  # noqa: E501
    ...
    def test_get_purchase_by_invoice_number(self, manager, mock_db):
        mock_db.fetch_one.return_value = (
            1,
            "INV001",
        )  # Need valid row for _row_to_purchase basic check

        # We need to mock _row_to_purchase to avoid IndexError if the tuple above is too short
        # Or provide a full tuple.
        # Let's mock the helper method entirely for simplicity here, as we tested _row_to_purchase implicitly in test_get_purchase_by_id  # noqa: E501

        with patch.object(
            manager,
            "_row_to_purchase",
            return_value=Purchase(id=1, invoice_number="INV001"),
        ), patch.object(
            manager,
            "get_purchase_by_id",
            return_value=Purchase(id=1, invoice_number="INV001"),
    ...
    def test_list_purchases(self, manager, mock_db):
        ...
        mock_db.fetch_all.return_value = [(1, "INV001", "S1", "2023-01-01", 100.0, 0.0, 100.0, "معلقة", "غير مدفوعة")]
