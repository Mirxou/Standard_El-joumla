...
def test_create_customer_with_optional_columns_and_webhook_ignored_on_import_error(
    monkeypatch,
):
    db = DummyDB()
    db.next_id = 42
...
from src.models.customer import Customer  # noqa: F811
...
class TestCustomerDataclass(unittest.TestCase):
    ...
    def test_total_purchases_conversion(self):
        ...
        customer = Customer(total_purchases=15000.50, purchases_count=5)
...
class TestCustomerProperties(unittest.TestCase):
    ...
    def test_full_address_all_parts(self):
        ...
        customer = Customer(address="شارع النيل 123", city="الجزائر العاصمة", country="الجزائر")
    ...
    def test_full_address_partial(self):
        ...
        customer = Customer(address="شارع النيل 123", city="الجزائر العاصمة", country=None)
    ...
    def test_full_address_none_all(self):
        ...
        customer = Customer(address=None, city=None, country=None)
...
class TestCustomerToDict(unittest.TestCase):
    ...
    def test_to_dict_basic(self):
        ...
        customer = Customer(id=1, name="محمد علي", phone="0123456789")
...
class TestCustomerEdgeCases(unittest.TestCase):
    ...
    def test_customer_with_unicode_names(self):
        ...
        customer = Customer(name="محمد علي", name_en="Mohamed Ali", address="شارع 123")
    ...
    def test_customer_with_large_credit(self):
        """ائتمان كبير جداً"""
        customer = Customer(credit_limit=Decimal("999999999.99"), current_balance=Decimal("100000.00"))
        self.assertEqual(customer.available_credit, Decimal("999899999.99"))
        self.assertFalse(customer.is_credit_exceeded)

    def test_customer_with_multiple_phones(self):
        """عميل بهاتفين"""
        customer = Customer(phone="0123456789", phone2="0987654321")
        self.assertEqual(customer.phone, "0123456789")
        self.assertEqual(customer.phone2, "0987654321")

    def test_customer_inactive(self):
        """عميل غير مفعل"""
        customer = Customer(name="عميل غير مفعل", is_active=False)
        self.assertFalse(customer.is_active)

    def test_customer_with_tax_number(self):
        """عميل برقم ضريبي"""
        customer = Customer(tax_number="TN0123456789")
...
class TestCustomerCredit(unittest.TestCase):
    ...
    def test_credit_utilization_zero(self):
        """استخدام ائتمان صفر"""
        customer = Customer(credit_limit=Decimal("10000"), current_balance=Decimal("0"))
    ...
    def test_credit_utilization_full(self):
        ...
        customer = Customer(credit_limit=Decimal("10000"), current_balance=Decimal("10000"))
        self.assertEqual(customer.available_credit, Decimal("0"))
        self.assertFalse(customer.is_credit_exceeded)

    def test_credit_exceeded_small_amount(self):
        """تجاوز ائتمان صغير"""
        customer = Customer(credit_limit=Decimal("10000"), current_balance=Decimal("10001"))
        self.assertTrue(customer.is_credit_exceeded)
        self.assertEqual(customer.available_credit, Decimal("-1"))

    def test_credit_exceeded_large_amount(self):
        """تجاوز ائتمان كبير"""
        customer = Customer(credit_limit=Decimal("10000"), current_balance=Decimal("15000"))
        self.assertTrue(customer.is_credit_exceeded)
        self.assertEqual(customer.available_credit, Decimal("-5000"))
