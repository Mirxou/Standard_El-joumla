...
class TestSaleManager:
    ...
    def test_create_sale_no_id_fallback(self, manager, mock_db):
        ...
        with patch.object(manager, "_create_sale_item"), patch.object(manager, "_update_stock_for_sale"):
    ...
    def test_update_sale(self, manager, mock_db):
        ...
        with patch.object(manager, "get_sale_by_id") as mock_get:
            ...
            with patch.object(manager, "_update_stock_in_transaction"), patch.object(
                manager, "_create_sale_item_in_transaction"
            ):
    ...
    def test_update_sale_fallback(self, manager, mock_db):
        ...
        with patch.object(manager, "get_sale_by_id", return_value=Sale(id=1, items=[])), patch.object(
            manager, "_update_stock_in_transaction"
    ...
    def test_delete_sale_soft(self, manager, mock_db):
        ...
        with patch.object(manager, "get_sale_by_id", return_value=sale), patch.object(
            manager, "_update_stock_for_sale"
        ) as mock_stock:
    ...
    def test_cancel_sale_failure(self, manager, mock_db):
        ...
        sale = Sale(id=1)
        with patch.object(manager, "get_sale_by_id", return_value=sale), patch.object(
            manager, "update_sale_status", return_value=False
        ):
            assert manager.cancel_sale(1) is False
    ...
    def test_create_sale_recovery_by_invoice(self, manager, mock_db):
        ...
        with patch.object(manager, "_create_sale_item", return_value=1), patch.object(
            manager, "_update_stock_for_sale"
        ):
