...
from src.models.user import Permission, User, UserManager, UserRole
...
class TestUserCreation(unittest.TestCase):
    ...
    def test_user_basic_creation(self):
        """إنشاء مستخدم أساسي"""
        user = User(username="ahmed", email="ahmed@example.com", full_name="أحمد محمد")
    ...
    def test_user_with_phone(self):
        """مستخدم مع رقم هاتف"""
        user = User(username="test", phone="+966123456789")
        self.assertEqual(user.phone, "+966123456789")

    def test_user_with_role(self):
        """مستخدم مع دور محدد"""
        user = User(username="test", role=UserRole.ADMIN.value)
...
class TestUserAccountManagement(unittest.TestCase):
    ...
    def test_user_with_password_hash(self):
        ...
        user = User(username="test", password_hash="hashed_password_123", salt="salt_value")
    ...
    def test_user_locked_account(self):
        """حساب مستخدم مقفول"""
        user = User(username="test", is_locked=True)
    ...
    def test_user_last_login(self):
        """آخر تسجيل دخول"""
        now = datetime.now()
        user = User(username="test", last_login=now)
        self.assertEqual(user.last_login, now)

    def test_user_last_password_change(self):
        """آخر تغيير كلمة مرور"""
        now = datetime.now()
        user = User(username="test", last_password_change=now)
...
class TestUserEdgeCases(unittest.TestCase):
    ...
    def test_user_with_notes(self):
        """مستخدم مع ملاحظات"""
        user = User(username="test", notes="مستخدم تجريبي")
