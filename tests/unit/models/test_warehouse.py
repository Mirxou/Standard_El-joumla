...
def test_warehouse_manager_create_prevents_duplicates_and_sets_default(monkeypatch):
    ...
    monkeypatch.setattr(
        mgr,
        "_unset_default_warehouse",
        lambda exclude_id=None: unset_called.setdefault("called", True),
    )
...
def test_inventory_manager_basic_flows(monkeypatch):
    ...
    db.all_results = [
        {
            ...
            "max_stock": 10,
            "reorder_point": 2,
            "last_movement_date": "2024-01-02T00:00:00",
    ...
    monkeypatch.setattr(
        inv_mgr,
        "get_inventory",
        lambda w, p: WarehouseInventory(quantity=1.0, reserved_quantity=0.0, warehouse_id=w, product_id=p),
    )
...
def test_transfer_manager_generate_and_create(monkeypatch):
    ...
    assert (
        trans_mgr.create_transfer(WarehouseTransfer(from_warehouse_id=1, to_warehouse_id=2, product_id=3, quantity=5.0))
        is None
    )
    ...
    reserved = {}
    monkeypatch.setattr(
        trans_mgr.inventory_manager,
        "reserve_quantity",
        lambda w, p, q: reserved.setdefault("called", (w, p, q)) or True,
    )
    tid = trans_mgr.create_transfer(
        WarehouseTransfer(from_warehouse_id=1, to_warehouse_id=2, product_id=3, quantity=2.0)
    )
    assert tid == 1
...
from src.models.warehouse import Warehouse, WarehouseInventory  # noqa: F811
...
class TestWarehouse(unittest.TestCase):
    ...
    def test_warehouse_is_active(self):
        """مستودع نشط"""
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", is_active=True)
        self.assertTrue(warehouse.is_active)

    def test_warehouse_is_inactive(self):
        """مستودع غير نشط"""
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", is_active=False)
        self.assertFalse(warehouse.is_active)

    def test_warehouse_is_default(self):
        """مستودع افتراضي"""
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", is_default=True)
        self.assertTrue(warehouse.is_default)

    def test_warehouse_not_default(self):
        """مستودع غير افتراضي"""
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", is_default=False)
        self.assertFalse(warehouse.is_default)

    def test_warehouse_with_notes(self):
        """مستودع مع ملاحظات"""
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", notes="مستودع مجهز بأنظمة الأمان")
    ...
    def test_warehouse_default_country(self):
        ...
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي")
    ...
    def test_warehouse_with_id(self):
        """مستودع مع معرف"""
        warehouse = Warehouse(id=123, code="WH001", name="المستودع الرئيسي")
...
class TestWarehouseInventory(unittest.TestCase):
    ...
    def test_warehouse_inventory_creation(self):
        ...
        inventory = WarehouseInventory(warehouse_id=1, product_id=1, quantity=100.0)
    ...
    def test_warehouse_inventory_with_reserved(self):
        ...
        inventory = WarehouseInventory(warehouse_id=1, product_id=1, quantity=100.0, reserved_quantity=20.0)
        self.assertEqual(inventory.quantity, 100.0)
    ...
    def test_warehouse_inventory_zero_quantity(self):
        """مخزون بكمية صفر"""
        inventory = WarehouseInventory(warehouse_id=1, product_id=1, quantity=0.0)
        self.assertEqual(inventory.quantity, 0.0)

    def test_warehouse_inventory_with_id(self):
        """مخزون مع معرف"""
        inventory = WarehouseInventory(id=1, warehouse_id=1, product_id=1, quantity=100.0)
...
class TestWarehouseEdgeCases(unittest.TestCase):
    ...
    def test_warehouse_with_special_characters(self):
        ...
        warehouse = Warehouse(code="WH_001", name="المستودع #1 - الفرع الأول")
    ...
    def test_warehouse_with_long_address(self):
        ...
        warehouse = Warehouse(code="WH001", name="المستودع", address=address)
        self.assertIn("شارع النيل", warehouse.address)

    def test_warehouse_with_empty_optional_fields(self):
        """مستودع بحقول اختيارية فارغة"""
        warehouse = Warehouse(code="WH001", name="المستودع", address=None, city=None, notes=None)
    ...
    def test_multiple_warehouses(self):
        """عدة مستودعات"""
        warehouses = [Warehouse(code=f"WH{i:03d}", name=f"المستودع {i}") for i in range(1, 6)]
    ...
    def test_warehouse_multilingual_names(self):
        ...
        warehouse = Warehouse(code="WH001", name="المستودع الرئيسي", name_en="Main Warehouse")
