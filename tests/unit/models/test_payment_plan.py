...
class TestInstallmentAmounts(unittest.TestCase):
    ...
    def test_installment_principal(self):
        ...
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"))

        self.assertEqual(installment.principal_amount, Decimal("1000.00"))

    def test_installment_with_interest(self):
        """اختبار القسط مع الفائدة"""
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), interest_amount=Decimal("100.00"))
    ...
    def test_installment_with_late_fee(self):
        ...
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), late_fee=Decimal("50.00"))
    ...
    def test_installment_amount_paid(self):
        ...
        installment = PaymentInstallment(total_amount=Decimal("1150.00"), amount_paid=Decimal("575.00"))
...
class TestInstallmentStatus(unittest.TestCase):
    ...
    def test_installment_pending(self):
        """اختبار قسط معلق"""
        installment = PaymentInstallment(status=InstallmentStatus.PENDING)
    ...
    def test_installment_overdue(self):
        ...
        installment = PaymentInstallment(status=InstallmentStatus.OVERDUE, due_date=past_date)
    ...
    def test_installment_cancelled(self):
        """اختبار قسط ملغي"""
        installment = PaymentInstallment(status=InstallmentStatus.CANCELLED)

        self.assertEqual(installment.status, InstallmentStatus.CANCELLED)

    def test_installment_waived(self):
        """اختبار قسط معفي"""
        installment = PaymentInstallment(status=InstallmentStatus.WAIVED)
...
class TestInstallmentDates(unittest.TestCase):
    ...
    def test_installment_early_payment(self):
        ...
        installment = PaymentInstallment(due_date=due_date, payment_date=payment_date)
...
class TestInstallmentPayment(unittest.TestCase):
    ...
    def test_installment_payment_method(self):
        """اختبار طريقة الدفع"""
        installment = PaymentInstallment(payment_method="تحويل بنكي")

        self.assertEqual(installment.payment_method, "تحويل بنكي")

    def test_installment_payment_reference(self):
        """اختبار مرجع الدفع"""
        installment = PaymentInstallment(payment_reference="PAY-001-2024")
...
class TestInstallmentNotes(unittest.TestCase):
    ...
    def test_installment_notes(self):
        ...
        installment = PaymentInstallment(notes="تم التأجيل بسبب ظرف طارئ")
...
class TestInstallmentEdgeCases(unittest.TestCase):
    ...
    def test_installment_zero_amount(self):
        """اختبار مبلغ صفر"""
        installment = PaymentInstallment(principal_amount=Decimal("0.00"), total_amount=Decimal("0.00"))

        self.assertEqual(installment.principal_amount, Decimal("0.00"))

    def test_installment_large_amount(self):
        """اختبار مبلغ كبير"""
        installment = PaymentInstallment(principal_amount=Decimal("999999.99"), total_amount=Decimal("999999.99"))

        self.assertEqual(installment.principal_amount, Decimal("999999.99"))

    def test_installment_high_interest(self):
        """اختبار فائدة عالية"""
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), interest_amount=Decimal("500.00"))

        self.assertGreater(installment.interest_amount, Decimal("0"))

    def test_installment_no_interest(self):
        """اختبار بدون فائدة"""
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), interest_amount=Decimal("0.00"))

        self.assertEqual(installment.interest_amount, Decimal("0.00"))

    def test_installment_high_late_fee(self):
        """اختبار غرامة تأخير عالية"""
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), late_fee=Decimal("500.00"))

        self.assertGreater(installment.late_fee, Decimal("0"))

    def test_installment_no_late_fee(self):
        """اختبار بدون غرامة تأخير"""
        installment = PaymentInstallment(principal_amount=Decimal("1000.00"), late_fee=Decimal("0.00"))

        self.assertEqual(installment.late_fee, Decimal("0.00"))

    def test_installment_overpayment(self):
        """اختبار الدفع الزائد"""
        installment = PaymentInstallment(total_amount=Decimal("1150.00"), amount_paid=Decimal("1200.00"))
