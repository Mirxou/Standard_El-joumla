...
class TestCacheServiceInitialization:
    ...
    def test_initialization_starts_cleanup_thread(self):
        ...
        # عد الخيوط قبل إنشاء الخدمة
        threads_before = threading.active_count()

        service = CacheService()

        # التحقق من أن خيط التنظيف قد بدأ (عدد الخيوط يجب أن يزيد)
        threads_after = threading.active_count()
        assert threads_after >= threads_before

        # التحقق من أن الخدمة تم إنشاؤها بنجاح
        assert service.caches is not None
        assert len(service.caches) > 0
...
class TestCleanupThread:
    ...
    def test_cleanup_thread_start(self):
        ...
        with patch("threading.Thread") as mock_thread:
            ...
            service = CacheService()  # noqa: F841
