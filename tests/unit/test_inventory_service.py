...
def service(mock_db_manager):
    ...
    with patch("src.services.inventory_service.ProductManager") as MockProductManager, patch(
        "src.services.inventory_service.CategoryManager"
    ) as MockCategoryManager, patch("src.services.inventory_service.SupplierManager") as MockSupplierManager:
...
class TestInventoryService:
    ...
    def test_transfer_stock_insufficient(self, service):
        ...
        service.product_manager.get_product_by_id.side_effect = [
            from_product,
            to_product,
        ]
    ...
    def test_search_products(self, service):
        ...
        query = "test"
        service.product_manager.search_products.return_value = [Product(id=1, name="Test Product")]

        results = service.search_products(query)

        assert len(results) == 1
        service.product_manager.search_products.assert_called_once_with(query, None, True, None)
    ...
    def test_get_stock_alerts(self, service, mock_db_manager):
        ...
        service.db_manager.fetch_all.return_value = [(1, "Low Stock", 5, 10)]  # (id, name, current_stock, min_stock)
    ...
    def test_transfer_stock_success(self, service):
        ...
        with patch.object(service, "is_multi_warehouse_enabled", return_value=False):
            with patch.object(service, "_record_stock_movement") as mock_record:  # noqa: F841
                result = service.transfer_stock(from_id, to_id, qty)
    ...
    def test_search_products_with_filters(self, service):
        ...
        service.product_manager.search_products.return_value = [Product(id=1, name="Test Product", category_id=1)]

        results = service.search_products(query, category_id=category_id, supplier_id=supplier_id, active_only=False)

        assert len(results) == 1
        service.product_manager.search_products.assert_called_once_with(query, category_id, False, None)
    ...
    def test_generate_inventory_report_with_movements(self, service):
        ...
        service.product_manager.get_low_stock_products.return_value = []
        service.db_manager.fetch_one.return_value = (10, 8, 2, 1000.0, 50.0)
        service.db_manager.fetch_all.return_value = [{"id": 1, "product_id": 1, "movement_type": "in", "quantity": 10}]
...
class TestInventoryWarehouseOperations:
    ...
    def test_transfer_stock_with_warehouse(self, service):
        """اختبار نقل المخزون مع تحديد المستودع"""
        product_id = 1
        warehouse_id = 1
        new_qty = 50

        # محاكاة أن الـ multi-warehouse غير مفعل حالياً للتبسيط أو محاكاته بالكامل
        with patch.object(service, "is_multi_warehouse_enabled", return_value=False):
            with patch.object(service, "adjust_stock", return_value=True) as mock_adjust:
                result = service.adjust_stock(product_id, new_qty, warehouse_id=warehouse_id)
                assert result is True
                mock_adjust.assert_called_once_with(product_id, new_qty, warehouse_id=warehouse_id)
