...
class TestAdvancedPredictionSystem:
    ...
    def test_initialize_system_with_sales_data(self, system):
        ...
        with patch.object(system.sales_model, 'train', return_value={"status": "trained", "accuracy": 0.9}):
    ...
    def test_initialize_system_with_inventory_data(self, system):
        ...
        historical_data = {
            "inventory": [
                {"product_id": "P001", "data": [{"date": "2025-01-01", "quantity": 100}]},
                {"product_id": "P002", "data": [{"date": "2025-01-01", "quantity": 50}]}
            ]
        }
        
        with patch.object(system.inventory_model, 'train_for_product', return_value={"status": "trained"}):
            result = system.initialize_system(historical_data)
    ...
    def test_generate_comprehensive_forecast(self, system):
        ...
        # محاكاة التنبؤات
        with patch.object(system.sales_model, 'predict_range', return_value={"forecast": [100, 200]}), \
             patch.object(system, '_generate_inventory_forecast', return_value={}), \
             patch.object(system.customer_model, 'predict_customer_behavior', return_value={}), \
             patch.object(system.market_model, 'predict_market_trends', return_value={}):
    ...
    def test_generate_forecast_with_context(self, system):
        ...
        with patch.object(system.sales_model, 'predict_range', return_value={}), \
             patch.object(system, '_generate_inventory_forecast', return_value={}), \
             patch.object(system.customer_model, 'predict_customer_behavior', return_value={}):
            
            result = system.generate_comprehensive_forecast(
