...
class TestIntelligentForecastingService:
    ...
    def service(self, db_manager):
        with patch(
            "src.services.intelligent_forecasting_service.setup_logger",
            return_value=MagicMock(),
        ):
            with patch(
                "src.services.intelligent_forecasting_service.CognitiveAIService",
                return_value=MagicMock(),
            ):
                with patch(
                    "src.services.intelligent_forecasting_service.AdvancedAnalyticsService",
                    return_value=MagicMock(),
                ):
