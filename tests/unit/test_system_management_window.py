...
class TestSystemManagementWindow:
    ...
    def window(self, qapp):
        ...
        mock_backup.get_backup_statistics.return_value = {
            'total_backups': 0, 'total_size': 0, 'newest_backup': None, 'oldest_backup': None
        }
        
        mock_perf = Mock()
        mock_perf.analyze_performance.return_value = {
            'database_size_mb': 0, 'table_count': 0, 'index_count': 0, 'freelist_count': 0,
            'fragmentation_percent': 0, 'largest_tables': []
        }
