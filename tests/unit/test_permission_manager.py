...
from src.core.permission_manager import Permission, PermissionManager, Role, User
...
class TestRole:
    ...
    def test_role_init(self):
        """اختبار تهيئة دور"""
        role = Role(role_id=1, name="admin", description="Administrator role")
...
class TestUser:
    ...
    def test_user_init(self):
        """اختبار تهيئة مستخدم"""
        user = User(user_id=1, username="test_user", role_id=1)
...
class TestPermissionManager:
    ...
    def test_user_has_permission(self, permission_manager):
        ...
        try:
            has_permission = permission_manager.user_has_permission(user_id=1, permission=Permission.SALES_VIEW.value)
