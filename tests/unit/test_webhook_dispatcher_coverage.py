...
from src.core.webhook_dispatcher import WebhookDispatcher
...
class TestWebhookDispatcherCoverage:
    ...
    def test_check_rate_limit(self, dispatcher):
        ...
        for _ in range(limit):
            assert dispatcher._check_rate_limit(webhook_id, limit) is True

        # 2. إرسال طلب فوق الحد (يجب أن يرفض)
        assert dispatcher._check_rate_limit(webhook_id, limit) is False
    ...
    def test_serialize_payload_error(self, dispatcher):
        ...
        result = dispatcher._deliver_webhook_sync(url="http://test.com", payload=payload, callback=callback_mock)
    ...
    def test_deliver_webhook_sync_connection_error(self, mock_request, dispatcher):
        ...
        result = dispatcher._deliver_webhook_sync(url="http://test.com", payload={}, retry_count=1)
    ...
    def test_deliver_webhook_sync_client_error(self, mock_request, dispatcher):
        ...
        result = dispatcher._deliver_webhook_sync(url="http://test.com", payload={}, retry_count=3)
    ...
    def test_deliver_webhook_sync_all_retries_fail(self, mock_sleep, mock_request, dispatcher):
        ...
        result = dispatcher._deliver_webhook_sync(url="http://test.com", payload={}, retry_count=2)
