    ...
    def service(self, db_manager):
        with patch("src.services.report_exporter.setup_logger", return_value=MagicMock()):
            with patch("src.services.report_exporter.DatabaseLogger", return_value=MagicMock()):
                with patch(
                    "src.services.report_exporter.ProductManager",
                    return_value=MagicMock(),
                ):
                    with patch(
                        "src.services.report_exporter.SaleManager",
                        return_value=MagicMock(),
                    ):
                        with patch(
                            "src.services.report_exporter.PurchaseManager",
                            return_value=MagicMock(),
                        ):
                            with patch(
                                "src.services.report_exporter.CustomerManager",
                                return_value=MagicMock(),
                            ):
                                with patch(
                                    "src.services.report_exporter.SupplierManager",
                                    return_value=MagicMock(),
                                ):
