...
class MockAuditLog:
    ...
    def __init__(self, id=None, user_id=None, username="", action="", 
                 resource_type="", resource_id=None, old_value=None, 
                 new_value=None, ip_address="", user_agent="", 
                 status="success", error_message="", created_at=None):
        self.id = id
...
class MockLoginHistory:
    ...
    def __init__(self, id=None, user_id=None, username="", login_time=None,
                 ip_address="", user_agent="", success=True, failure_reason=None,
...
class MockAuditService:
    ...
    def list_audit_logs(self, user_id: Optional[int] = None,
                       ...
                       limit: int = 100,
                       offset: int = 0) -> List[MockAuditLog]:
    ...
    def log_login_attempt(self, user_id: Optional[int], username: str,
                         ip_address: str, user_agent: str,
                         success: bool, failure_reason: Optional[str] = None) -> int:
        """Log a login attempt"""
        try:
            result = self.db.execute_update(
                """INSERT INTO login_history 
                   ...
                   VALUES (?, ?, ?, ?, ?, ?, ?)""",
                [user_id, username, datetime.now().isoformat(), ip_address, user_agent, success, failure_reason]
            )
            return result
        except Exception as e:
            if self.logger:
    ...
    def get_login_history(self, user_id: Optional[int] = None,
                         ...
                         start_date: Optional[datetime] = None,
                         end_date: Optional[datetime] = None,
                         limit: int = 100) -> List[MockLoginHistory]:
        ...
        try:
            ...
            history = []
            for row in result:
                history.append(MockLoginHistory(
                    id=row.get('id'),
                    user_id=row.get('user_id'),
                    username=row.get('username'),
                    ...
                    logout_time=datetime.fromisoformat(row.get('logout_time')) if row.get('logout_time') else None,
...
class TestListAuditLogs:
    ...
    def service_with_mocks(self):
        ...
        mock_db = Mock()
        mock_db.execute_query.return_value = [
            {'id': 1, 'user_id': 1, 'username': 'user1', 'action': 'create', 'resource_type': 'product', 'created_at': datetime.now().isoformat()},
            {'id': 2, 'user_id': 2, 'username': 'user2', 'action': 'update', 'resource_type': 'customer', 'created_at': datetime.now().isoformat()}
        ]
...
class TestGetLoginHistory:
    ...
    def test_get_login_history_success(self):
        ...
        mock_db = Mock()
        mock_db.execute_query.return_value = [
            {'id': 1, 'user_id': 1, 'username': 'user1', 'success': 1, 'login_time': datetime.now().isoformat()},
            {'id': 2, 'user_id': 1, 'username': 'user1', 'success': 0, 'login_time': datetime.now().isoformat()}
        ]
