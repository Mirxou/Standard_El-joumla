...
from src.models.user import Permission, User, UserRole
from src.services.user_service import SecuritySettings, UserService
...
class TestUserService:
    ...
    def test_validate_session_success(self, service):
        ...
        session = service._create_session(
            User(id=1, username="testuser", role=UserRole.ADMIN),
            "127.0.0.1",
            "pytest-agent",
        )
    ...
    def test_validate_session_expired(self, service):
        ...
        with patch.object(service, "security_settings") as mock_settings:
            ...
            session = service._create_session(  # noqa: F841
                User(id=1, username="testuser", role=UserRole.ADMIN),
                "127.0.0.1",
                "pytest-agent",
            )  # noqa: F841
    ...
    def test_logout_user(self, service):
        ...
        session = service._create_session(
            User(id=1, username="testuser", role=UserRole.CASHIER),
            "127.0.0.1",
            "pytest-agent",
        )
        session_id = session.session_id
    ...
    def test_change_password_weak_password(self, service, mock_user_manager):
        ...
        with patch.object(
            ...
            return_value=(False, "كلمة المرور ضعيفة"),
        ):
