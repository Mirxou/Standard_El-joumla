...
try:
    ...
    cursor.execute("""
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        'TEST_MODEL', 'Test Model', 'cognitive', 'test', 'test_algo', 0.8,
        'trained', '2024-01-01', '/tmp/model.pkl', json.dumps({'param': 'value'}),
        json.dumps({'metric': 'value'}), json.dumps([1,2,3]), json.dumps([[1,0],[0,1]]),
        json.dumps([0.8, 0.85, 0.82]), json.dumps({'C': 1.0}), json.dumps(['feat1', 'feat2']),
        '2024-01-01'
    ))
