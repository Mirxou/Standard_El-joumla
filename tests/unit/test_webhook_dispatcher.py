...
from src.core.webhook_dispatcher import WebhookDeliveryResult, WebhookDispatcher
...
class TestWebhookDispatcher:
    ...
    def mock_payload(self):
        """Payload تجريبي"""
        return {"event": "test_event", "data": {"id": 1, "name": "Test"}}
    ...
    def test_should_retry_status_code(self, dispatcher):
        ...
        assert dispatcher._should_retry(500) is True
        assert dispatcher._should_retry(502) is True
        assert dispatcher._should_retry(503) is True
        assert dispatcher._should_retry(504) is True

        # لا يجب إعادة المحاولة للأخطاء 4xx
        assert dispatcher._should_retry(400) is False
        assert dispatcher._should_retry(404) is False
        assert dispatcher._should_retry(401) is False

        # يجب إعادة المحاولة للنجاح (لا يجب، لكن للاختبار)
        assert dispatcher._should_retry(200) is False
    ...
    def test_should_retry_exception(self, dispatcher):
        ...
        assert dispatcher._should_retry(None, requests.exceptions.ConnectionError()) is True
        assert dispatcher._should_retry(None, requests.exceptions.Timeout()) is True

        # لا يجب إعادة المحاولة للأخطاء الأخرى
        assert dispatcher._should_retry(None, ValueError()) is False
    ...
    def test_webhook_delivery_result(self):
        ...
        # Test failure result
        result_fail = WebhookDeliveryResult(success=False, error_message="Connection Error", attempt_number=3)
...
class TestWebhookQueue:
    ...
    def test_deliver_webhook_adds_to_queue(self, dispatcher):
        ...
        dispatcher.deliver_webhook(url="https://example.com/webhook", payload=payload, priority=5)
...
class TestWebhookShutdown:
    ...
    def test_shutdown_dispatcher(self, dispatcher):
        ...
        # إضافة بعض Webhooks
        for i in range(3):
            dispatcher.deliver_webhook(url=f"https://example.com/webhook{i}", payload={"event": "test"})
