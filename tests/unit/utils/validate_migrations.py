            ...
            if "ALTER TABLE" in content and "IF NOT EXISTS" not in content:
                ...
                if "ADD COLUMN" in content:
                    issues["sql_errors"].append(
                        {
                            "file": migration_file.name,
                            "issue": "ALTER TABLE ADD COLUMN قد يفشل إذا كان العمود موجوداً (SQLite لا يدعم IF NOT EXISTS)",  # noqa: E501
                        }
                    )

        except Exception as e:
            issues["sql_errors"].append({"file": migration_file.name, "issue": f"خطأ في قراءة الملف: {e}"})
...
def suggest_renaming(migrations_dir: Path) -> List[Tuple[str, str]]:
    ...
    suggestions = []
    migration_files = sorted(migrations_dir.glob("*.sql"))  # noqa: F841
