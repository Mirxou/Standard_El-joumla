...
def clean_test_crash_reports(logs_dir: Path = None, days_old: int = 7, force_delete_tests: bool = True):
    ...
    # print("🔍 البحث عن تقارير الأعطال من الاختبارات...")
    if force_delete_tests:
        # print("🗑️  وضع الحذف القسري: سيتم حذف جميع ملفات الاختبارات")
        pass
    else:
    ...
    for crash_file in crash_reports_dir.glob("*.txt"):
        try:
            ...
            is_test_file = (
                "test_exception_handler" in content
                or "StandardElJoumlaError" in content
                or "Test error" in content
                or "tests/unit" in content
            )
...
def clean_old_log_files(logs_dir: Path = None, days_old: int = 30):
    ...
    keep_files = {"__main__.log", "database_operations.log", "exception_handler.log"}
