...
class TestGetSalesByCategory:
    ...
    def service_with_mock_db(self):
        mock_db = Mock()
        mock_db.fetch_all.return_value = [
            {'category_name': 'Electronics', 'sale_count': 10, 'total_quantity': 50, 'total_amount': 5000.0},
            {'category_name': 'Clothing', 'sale_count': 15, 'total_quantity': 30, 'total_amount': 3000.0},
        ]
        return AnalyticsService(db_manager=mock_db)
    ...
    def test_get_sales_by_category_with_null_category(self):
        mock_db = Mock()
        mock_db.fetch_all.return_value = [
            {'category_name': None, 'sale_count': 5, 'total_quantity': 10, 'total_amount': 1000.0},
        ]
        service = AnalyticsService(db_manager=mock_db)
...
class TestGetSalesByCustomer:
    ...
    def service_with_mock_db(self):
        mock_db = Mock()
        mock_db.fetch_all.return_value = [
            {'id': 1, 'customer_name': 'John Doe', 'sale_count': 5, 'total_amount': 5000.0, 'avg_amount': 1000.0},
        ]
        return AnalyticsService(db_manager=mock_db)
