...
class TestDatabaseMetrics:
    ...
    def test_metrics_get_recent_errors(self, metrics):
        ...
        # تسجيل عدة أخطاء
        for i in range(12):
            metrics.record_query(f"SELECT error_{i}", 50.0, "SELECT", success=False, error=f"Error {i}")
        ...
        def record_queries(start, count):
            ...
            try:
                for i in range(count):
                    metrics.record_query(f"SELECT {start + i}", 50.0, "SELECT")
    ...
    def test_metrics_statistics_error_rate_calculation(self, metrics):
        ...
        # 5 استعلامات فاشلة
        for i in range(5):
            metrics.record_query(f"SELECT error_{i}", 50.0, "SELECT", success=False, error="Error")
