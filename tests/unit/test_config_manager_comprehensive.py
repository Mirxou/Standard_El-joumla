...
class TestConfigManagerInitialization:
    ...
    def test_init_default_values(self):
        ...
        assert config._encryption_enabled is False
...
class TestConfigManagerLoadAndSave:
    ...
    def test_load_config_from_existing_file(self, temp_config_dir):
        ...
        test_config = {"database": {"path": "test.db"}, "ui": {"language": "ar"}}
...
class TestConfigManagerGetAndSet:
    ...
    def config(self):
        ...
        config = ConfigManager()
        config.config = {
            "database": {"path": "test.db", "pool_size": 10},
            "ui": {"language": "ar", "theme": "dark"},
        }
        return config
...
class TestConfigManagerEnvironmentVariables:
    ...
    def test_env_variable_string_conversion(self, config):
        ...
        try:
            ...
            int_value = config.get("test.int", use_env=True)  # noqa: F841
            bool_value = config.get("test.bool", use_env=True)  # noqa: F841
