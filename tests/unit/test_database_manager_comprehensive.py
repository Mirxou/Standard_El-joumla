...
class TestDatabaseManagerCRUD:
    ...
    def test_execute_query_insert(self, db):
        ...
        result = db.execute_query("INSERT INTO test_table (name, value) VALUES (?, ?)", ("test_item", 100))
        assert result is not None

    def test_execute_query_select(self, db):
        """اختبار SELECT باستخدام execute_query"""
        # إضافة بيانات أولاً
        db.execute_query("INSERT INTO test_table (name, value) VALUES (?, ?)", ("test_item", 100))
    ...
    def test_execute_insert(self, db):
        ...
        row_id = db.execute_insert("INSERT INTO test_table (name, value) VALUES (?, ?)", ("test_item", 100))
    ...
    def test_execute_non_query_update(self, db):
        ...
        # تحديث البيانات
        affected = db.execute_non_query("UPDATE test_table SET value = ? WHERE id = ?", (200, row_id))
...
class TestDatabaseManagerTransactions:
    ...
    def test_get_cursor_context_manager(self, db):
        ...
        with db.get_cursor() as cursor:
            cursor.execute("INSERT INTO test_table (name, value) VALUES (?, ?)", ("test_item", 100))
    ...
    def test_transaction_rollback_on_exception(self, db):
        ...
        result = db.fetch_one("SELECT * FROM test_table WHERE name = ?", ("test_item",))  # noqa: F841
...
class TestDatabaseManagerConnectionPool:
    ...
    def db(self):
        ...
        db = DatabaseManager(db_path=db_path, pool_options={"enabled": True, "pool_size": 5})
        db.initialize()
...
class TestDatabaseManagerUtilities:
    ...
    def test_vacuum_database(self, db):
        ...
        for i in range(10):
            db.execute_query("INSERT INTO test_table (name) VALUES (?)", (f"item_{i}",))
...
class TestDatabaseManagerEdgeCases:
    ...
    def test_large_query(self, db):
        ...
        # إدراج بيانات كبيرة
        large_data = "x" * 10000
        db.execute_query("INSERT INTO test_table (data) VALUES (?)", (large_data,))
    ...
    def test_concurrent_access(self, db):
        ...
        for i in range(10):
            db.execute_query("INSERT INTO test_table (value) VALUES (?)", (i,))
