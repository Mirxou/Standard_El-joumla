...
from src.services.ai_analytics_engine import AIAnalyticsEngine, SalesPrediction, CustomerInsight, ProductRecommendation
from src.services.sales_prediction_service import SalesPredictionService, DemandForecast, SeasonalPattern, InventoryRecommendation
from src.services.customer_behavior_analytics_service import CustomerBehaviorAnalyticsService, CustomerSegment, CustomerJourney, ChurnPrediction, CustomerLifetimeValue
...
class TestAIAnalyticsEngine(unittest.TestCase):
    ...
    def test_analyze_customer_behavior(self, mock_data):
        ...
        for insight in insights:
            ...
            self.assertIn(insight.insight_type, ['churn_risk', 'upsell_opportunity', 'loyalty_score'])
    ...
    def test_recommend_products(self, mock_products, mock_history):
        ...
        mock_history.return_value = [
            {'product_id': 1, 'product_name': 'Product A', 'total_quantity': 5, 'avg_price': 100, 'last_purchase': datetime.now()}
        ]
        mock_products.return_value = [
            {'id': 2, 'name': 'Product B', 'category_id': 1}
        ]
...
class TestCustomerBehaviorAnalyticsService(unittest.TestCase):
    ...
    @patch('services.customer_behavior_analytics_service.CustomerBehaviorAnalyticsService._get_customer_segmentation_data')
    ...
    def test_analyze_customer_journey(self, mock_data):
        ...
        mock_data.return_value = [
            {'date': datetime.now() - timedelta(days=30), 'amount': 200, 'item_count': 2, 'categories': ['1']},
            {'date': datetime.now() - timedelta(days=15), 'amount': 500, 'item_count': 5, 'categories': ['1', '2']},
            {'date': datetime.now() - timedelta(days=5), 'amount': 300, 'item_count': 3, 'categories': ['2']}
        ]
