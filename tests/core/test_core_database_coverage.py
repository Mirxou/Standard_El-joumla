...
from src.core.database_manager import ConnectionPool
...
class TestDatabaseManager:
    ...
    def legacy_manager(self, tmp_path):
        ...
        with patch('src.core.database_manager.sqlite3') as mock_sqlite, \
             patch('src.database.sqlite_backend.sqlite3') as mock_backend_sqlite, \
             patch('src.core.database_manager.ConnectionPool') as mock_pool_cls, \
             patch('src.core.database_manager.sqlite3') as mock_pool_sqlite, \
             patch('src.core.database_manager.EncryptionManager') as mock_enc_mgr, \
             patch('src.core.database_manager.EncryptedBackupService') as mock_backup_svc:
             ...
             with patch.object(manager, '_create_tables'), \
                  patch.object(manager, '_upgrade_existing_schema'), \
                  patch.object(manager, '_create_indexes'), \
                  patch.object(manager, '_run_migrations'), \
                  patch.object(manager, 'check_and_migrate_db'):
                 manager.initialize()
    ...
    def test_fetch_one(self, legacy_manager):
        legacy_manager.connection.cursor.return_value.fetchone.return_value = (1, 'test')
