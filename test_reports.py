...
# 5. اختبار أعلى الموردين رصيداً
print("\n5️⃣  اختبار أعلى الموردين رصيداً:")
print("-" * 80)
try:
    top_balance = db_manager.fetch_all(
        "SELECT name, COALESCE(current_balance, 0) as balance FROM suppliers WHERE COALESCE(current_balance, 0) > 0 ORDER BY balance DESC LIMIT 5"
    )
    if top_balance:
...
print("\n" + "=" * 80)
print("✓ انتهى الاختبار بنجاح - جميع التقارير تعمل بدون أخطاء")
print("=" * 80)
