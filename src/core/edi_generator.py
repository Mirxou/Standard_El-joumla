...
class EDIGenerator:
    ...
    def generate(self, document_type: str, data: Dict[str, Any], 
                 sender_id: str = "", receiver_id: str = "") -> EDIGenerateResult:
        """
        توليد محتوى EDI
    ...
    def _generate_edifact(self, doc_type: str, data: Dict[str, Any],
                          sender_id: str, receiver_id: str) -> EDIGenerateResult:
        """توليد EDIFACT"""
        try:
            lines = []
            ...
            lines.append(f"UNB+UNOA:2+{sender_id}+{receiver_id}+{datetime.now().strftime('%Y%m%d:%H%M')}+{interchange_ref}'")
            ...
            return EDIGenerateResult(
                success=False,
                errors=[f"خطأ في توليد EDIFACT: {str(e)}"]
            )
    
    def _generate_x12(self, doc_type: str, data: Dict[str, Any],
                      sender_id: str, receiver_id: str) -> EDIGenerateResult:
        """توليد X12"""
        try:
            lines = []
            ...
            isa_ref = "000000001"
            lines.append(f"ISA*00*          *00*          *ZZ*{sender_id:<15}*ZZ*{receiver_id:<15}*{isa_date}*{isa_time}*^*00501*{isa_ref}*0*P*:~")
            ...
            segment_count = len([l for l in lines if l.startswith(("ST", "BEG", "PO1", "CTT", "SE"))])
            ...
            return EDIGenerateResult(
                success=False,
                errors=[f"خطأ في توليد X12: {str(e)}"]
            )
    ...
    def generate_from_purchase_order(self, po_data: Dict[str, Any],
                                     sender_id: str, receiver_id: str) -> EDIGenerateResult:
    ...
    def generate_from_invoice(self, invoice_data: Dict[str, Any],
                              sender_id: str, receiver_id: str) -> EDIGenerateResult:
        """توليد EDI من فاتورة"""
