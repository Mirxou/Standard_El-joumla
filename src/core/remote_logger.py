...
class RemoteLogRunnable(BaseRunnable):
    ...
    def __init__(self, log_data: Dict[str, Any], api_client: APIClient, signals: Optional[RemoteLogSignals] = None):
        super().__init__()
    ...
    def run(self):
        """إرسال السجل"""
        try:
            response = self.api_client.post("/api/v1/logs", self.log_data)
            if self.signals:
