...
class IncrementalBackupService:
    ...
    def create_incremental_backup(self, base_snapshot_name: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if base_snapshot_name:
                ...
                cursor.execute("SELECT * FROM backup_snapshots ORDER BY created_at DESC LIMIT 1")
            ...
            cursor.execute(
                "INSERT INTO backup_snapshots (snapshot_name, backup_type, tables_hash, base_snapshot_id) VALUES (?, ?, ?, ?)",  # noqa: E501
                (
                    snapshot_name,
                    "incremental",
                    json.dumps(current_checksums),
                    base_snapshot_id,
                ),
            )
            snapshot_id = cursor.lastrowid
    ...
    def _calculate_table_checksums(self) -> Dict[str, Dict[str, Any]]:
        ...
        for table in tables:
            try:
                ...
                checksums[table] = {"checksum": checksum, "row_count": len(rows)}
            except Exception:
                # تخطي الجداول التي لا يمكن قراءتها
                logging.getLogger(__name__).warning("Ignored exception in incremental_backup_service.py")
    ...
    def list_backups(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            cursor.execute(
                "SELECT snapshot_name, backup_type, created_at, base_snapshot_id FROM backup_snapshots ORDER BY created_at DESC"  # noqa: E501
            )
