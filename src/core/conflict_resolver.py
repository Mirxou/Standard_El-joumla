...
class ConflictResolver:
    ...
    def resolve_conflict(
        ...
        local_version: int,
        remote_version: int,
        custom_merge: Optional[Dict[str, Any]] = None,
    ) -> ConflictResolution:
        """
        حل تعارض
        ...
        try:
            ...
            return ConflictResolution(success=True, strategy=strategy, resolved_data=resolved_data)

        except Exception as e:
            self.logger.error(f"❌ خطأ في حل التعارض: {e}", exc_info=True)
            return ConflictResolution(success=False, strategy=strategy, error_message=str(e))
    ...
    def _save_resolution(
        ...
        try:
            ...
            self.db_manager.execute_query(
                ...
                (
                    strategy.value,
                    json.dumps(resolved_data, ensure_ascii=False),
                    conflict_id,
                ),
            )
