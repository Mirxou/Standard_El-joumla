...
class BackupManager:
    ...
    def create_backup(self, description: str = "") -> Dict[str, Any]:
        ...
        try:
            ...
            logger.log(logging.ERROR, f"Backup failed: {str(e)}")
    ...
    def _verify_backup(self, backup_path: str) -> bool:
        ...
        try:
            ...
            logger.log(logging.ERROR, f"Backup verification error: {str(e)}")
            return False
    ...
    def restore_backup(self, backup_path: str, target_path: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "backup_path": backup_path, "target_path": target}

        except Exception as e:
            logger.log(logging.ERROR, f"Restore failed: {str(e)}")
            return {"success": False, "error": str(e)}
        ...
        for backup in backups:
            if backup not in keep_backups:
                try:
                    ...
                    logger.log(logging.ERROR, f"Failed to delete backup {backup.filename}: {str(e)}")
            ...
            try:
                ...
                if self._should_backup():
                    ...
                    if result["success"]:
                        ...
                        logger.log(logging.ERROR, f"Scheduled backup failed: {result.get('error')}")

                # الانتظار دقيقة قبل التحقق مجدداً
                self._scheduler_stop.wait(60)

            except Exception as e:
                logger.log(logging.ERROR, f"Scheduler error: {str(e)}")
                self._scheduler_stop.wait(60)
    ...
    def _load_last_backup_time(self):
        ...
        if os.path.exists(metadata_file):
            try:
                ...
                logger.log(logging.ERROR, f"Failed to load last backup time: {str(e)}")
    ...
    def _save_last_backup_time(self):
        ...
        try:
            ...
            logger.log(logging.ERROR, f"Failed to save last backup time: {str(e)}")
