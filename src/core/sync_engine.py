...
class SyncEngine:
    ...
    def calculate_hash(self, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            self.logger.error(f"❌ خطأ في حساب Hash: {e}", exc_info=True)
            return ""
    ...
    def detect_conflict(self, local_data: Dict[str, Any], remote_data: Dict[str, Any],
                       local_version: int, remote_version: int) -> bool:
        """
        اكتشاف التعارض
        ...
        try:
            ...
            self.logger.error(f"❌ خطأ في اكتشاف التعارض: {e}", exc_info=True)
            return False
    
    def sync_entity(self, entity_type: str, entity_id: int,
                   local_data: Dict[str, Any], remote_data: Optional[Dict[str, Any]],
        ...
        try:
            ...
            if direction == SyncDirection.UP or (direction == SyncDirection.BOTH and local_version > remote_version):
                ...
                success = True
            elif direction == SyncDirection.DOWN or (direction == SyncDirection.BOTH and remote_version > local_version):
                # تنزيل من السحابة
                status = SyncStatus.SUCCESS
                success = True
            else:
    ...
    def get_sync_state(self, sync_settings_id: int, entity_type: str, entity_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error(f"❌ خطأ في الحصول على حالة المزامنة: {e}", exc_info=True)
            return None
    
    def update_sync_state(self, sync_settings_id: int, entity_type: str, entity_id: int,
                         local_version: int, remote_version: int,
        ...
        try:
            ...
            self.logger.error(f"❌ خطأ في تحديث حالة المزامنة: {e}", exc_info=True)
