...
class SOXControlsManager:
    ...
    def _initialize_default_controls(self):
        ...
        try:
            ...
            try:
                ...
                logging.getLogger(__name__).warning("Ignored exception in sox_controls.py")
    ...
    def get_all_controls(self, company_id: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if isinstance(rows[0], dict):
                ...
                columns = (
                    [desc[0] for desc in self.db_manager.execute_query("PRAGMA table_info(sox_controls)").description]
                    if hasattr(
                        self.db_manager.execute_query("PRAGMA table_info(sox_controls)"),
                        "description",
                    )
                    else []
                )
        ...
        try:
            ...
            if row:
                return {
                    ...
                    "compliance_rate": (
                        (row.get("passed", 0) / row.get("total", 1) * 100) if row.get("total", 0) > 0 else 0
                    ),
                }
        ...
        try:
            ...
            if users_without_permissions > 0:
                self.test_control(
                    control_id,
                    "FAILED",
                    f"يوجد {users_without_permissions} مستخدم بدون صلاحيات",
                )
                return {
                    "status": "FAILED",
    ...
    def _test_change_management(self, control_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "status": "PASSED",
                "message": f"تم تسجيل {recent_changes} تغيير في آخر أسبوع",
            }
    ...
    def _test_data_integrity(self, control_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if invalid_sales > 0:
                ...
                return {
                    "status": "FAILED",
                    "message": f"يوجد {invalid_sales} عملية بيع بدون مبلغ",
                }
            else:
                self.test_control(control_id, "PASSED")
        ...
        try:
            ...
            if sales_without_customer > 0:
                self.test_control(
                    control_id,
                    "WARNING",
                    f"يوجد {sales_without_customer} عملية بيع بدون عميل",
                )
                return {
                    "status": "WARNING",
    ...
    def _test_audit_trail(self, control_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_logs == 0:
                ...
                return {
                    "status": "PASSED",
                    "message": f"تم تسجيل {recent_logs} سجل تدقيق في آخر 24 ساعة",
                }
