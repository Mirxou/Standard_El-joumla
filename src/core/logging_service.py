...
class AdvancedLoggingService:
                    ...
                    try:
                        if self.stream:
                            try:
                                ...
                                logging.getLogger(__name__).warning("Ignored exception in logging_service.py")
                    finally:
                        self.stream = None
                        self.release()
    ...
    def _create_performance_logger(self) -> logging.Logger:
        ...
        perf_handler = logging.handlers.RotatingFileHandler(
            perf_path, maxBytes=self.max_bytes, backupCount=5, encoding="utf-8"
        )
    ...
    def _log(self, level: int, message: str, extra: Dict[str, Any], exc_info: bool = False) -> None:
        ...
        # إنشاء record مخصص
        record = self.logger.makeRecord(self.logger.name, level, "(internal)", 0, message, (), None, extra=extra)
    ...
    def log_performance(self, operation: str, duration_ms: float, success: bool = True, **kwargs) -> None:
        """
        تسجيل مقاييس الأداء
...
if __name__ == "__main__":
    # إنشاء خدمة السجلات
    log_service = AdvancedLoggingService(app_name="TestApp", log_level="DEBUG")
