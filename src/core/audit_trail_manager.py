...
class AuditTrailManager:
    ...
    def log(
        ...
        try:
            ...
            try:
                ...
                logging.getLogger(__name__).warning("Ignored exception in audit_trail_manager.py")
            return None

    def _calculate_changes(
        self, old_values: Optional[Dict[str, Any]], new_values: Optional[Dict[str, Any]]
    ) -> Dict[str, tuple]:
        """
    ...
    def _update_activity_summary(self, user_id: int, action: str, success: bool):
        ...
        try:
            ...
            logger.log(logging.ERROR, f"Failed to update activity summary: {str(e)}")

    def get_entity_history(self, entity_type: str, entity_id: int, limit: int = 100) -> List[AuditEntry]:
        """
    ...
    def cleanup_old_records(self, days: int = 365) -> int:
        ...
        try:
            ...
            cursor = self.db.execute("DELETE FROM audit_trail WHERE timestamp < ?", (cutoff_date,))
            ...
            logger.log(logging.ERROR, f"Failed to cleanup old records: {str(e)}")
            return 0
