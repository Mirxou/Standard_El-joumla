...
class EDIParser:
    ...
    def parse(self, edi_content: str) -> EDIParseResult:
        ...
        try:
            if self.standard == "EDIFACT":
                ...
                return EDIParseResult(success=False, errors=[f"معيار غير مدعوم: {self.standard}"])
    ...
    def _parse_edifact(self, content: str) -> EDIParseResult:
        ...
        try:
            ...
            if not lines:
                return EDIParseResult(success=False, errors=["محتوى EDI فارغ"])
            ...
            if not document_type:
                return EDIParseResult(success=False, errors=["لم يتم العثور على نوع المستند في UNH"])

            # تحليل المقاطع
            parsed_data = self._parse_edifact_segments(segments, document_type)

            return EDIParseResult(success=True, document_type=document_type, data=parsed_data)

        except Exception as e:
            return EDIParseResult(success=False, errors=[f"خطأ في تحليل EDIFACT: {str(e)}"])
    ...
    def _parse_x12(self, content: str) -> EDIParseResult:
        ...
        try:
            ...
            if not document_type:
                return EDIParseResult(success=False, errors=["لم يتم العثور على نوع المستند في ST"])
            ...
            return EDIParseResult(success=False, errors=[f"خطأ في تحليل X12: {str(e)}"])

    def _parse_edifact_segments(self, segments: List[str], doc_type: str) -> Dict[str, Any]:
        """تحليل مقاطع EDIFACT"""
        data = {"document_type": doc_type, "header": {}, "items": [], "summary": {}}
            ...
            if segment.startswith("UNB+"):
                ...
                if len(parts) >= 3 and data["items"]:
                    ...
                    try:
                        ...
                        logging.getLogger(__name__).warning("Ignored exception in edi_parser.py")
    ...
    def _map_edifact_document_type(self, msg_type: str) -> str:
        ...
        mapping = {
            "ORDERS": "850",  # Purchase Order  # noqa: F601
            "INVOIC": "810",  # Invoice
            "ORDERS": "855",  # Purchase Order Acknowledgment  # noqa: F601
