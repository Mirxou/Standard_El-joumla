            ...
            try:
                ...
                if webhook_id and rate_limit:
                    if not self._check_rate_limit(webhook_id, rate_limit):
                        ...
                        self.logger.warning(
                            f"⚠️ Rate Limit تجاوز لـ Webhook {webhook_id}, "
    ...
    def deliver_webhook(
        ...
        if webhook_id and rate_limit_per_minute:
            if not self._check_rate_limit(webhook_id, rate_limit_per_minute):
                if self.logger:
                    self.logger.warning(
                        f"⚠️ تم تجاوز Rate Limit لـ Webhook {webhook_id}: "
                        f"{rate_limit_per_minute} طلب/دقيقة"
                    )
                return False
        ...
        self._delivery_queue.put((
            priority,  # الأولوية (1=عاجل, 5=عادي, 10=منخفض)
            time.time(),  # Timestamp للترتيب عند نفس الأولوية
            {
                "url": url,
                "payload": payload,
        ...
        with self._rate_limit_lock:
            ...
            if webhook_id in self._rate_limit_tracker:
                self._rate_limit_tracker[webhook_id] = [
                    ts for ts in self._rate_limit_tracker[webhook_id]
        ...
        **kwargs
        ...
        for attempt in range(1, retry_count + 1):
            try:
                ...
                self.logger.warning(
                    f"⚠️ Timeout في إرسال Webhook: {url} "
                ...
                self.logger.warning(
                    f"⚠️ Connection Error في إرسال Webhook: {url} "
                    f"(Attempt: {attempt}/{retry_count})"
                )
                ...
                self.logger.error(
                    f"❌ خطأ غير متوقع في إرسال Webhook: {url} "
            ...
            if attempt < retry_count:
                ...
                self.logger.debug(
                    f"⏳ انتظار {backoff_seconds:.2f} ثانية قبل إعادة المحاولة..."
                )
