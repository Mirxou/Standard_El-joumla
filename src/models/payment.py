...
class PaymentManager:
        ...
        try:
            ...
            try:
                amount_in_base_currency_val = (
                    ...
                    if payment.amount_in_base_currency is not None
                    else payment.amount
                )
            except Exception:
            ...
            if not insert_cols:
                ...
                minimal = [c for c in (
                    ...
                    'amount','currency','exchange_rate','amount_in_base_currency','payment_date','created_at','updated_at'
                ) if c in available]
                insert_cols = minimal
            ...
            try:
                ...
                try:
                    print(f"[PaymentManager.create_payment] execute error: {ex}\nQuery: {query}\nCols: {insert_cols}\nVals sample: {insert_vals[:6]}")
                except Exception:
                    pass
                raise
            ...
            if not payment_id:
                ...
                try:
                    ...
                    if row:
                        payment_id = row[0]
                        try:
                            print(f"[PaymentManager.create_payment] Retrieved payment_id={payment_id} via payment_number lookup")
                        except Exception:
                            pass
                    ...
                    if not payment_id:
                        ...
                        try:
                            if "created_at" in available and ("id" in available or "payment_id" in available):
                                ...
                                cursor.execute(
                                    f"SELECT {idc} FROM payments ORDER BY created_at DESC LIMIT 1"
                                )
                                r2 = cursor.fetchone()
                                if r2:
                                    payment_id = r2[0]
    ...
    def get_payment_by_id(self, payment_id: int) -> Optional[Payment]:
        ...
        try:
            ...
            conn = self.db_manager.connection if hasattr(self.db_manager, 'connection') and self.db_manager.connection else self.db_manager.get_connection()
    ...
    def _row_to_payment(self, row) -> Payment:
        ...
        base_amount = Decimal(str(row[10])) if row_len > 10 and row[10] is not None and not isinstance(row[10], str) else None
        converted_amount = Decimal(str(row[11])) if row_len > 11 and row[11] is not None and not isinstance(row[11], str) else None
        ...
        return Payment(
            ...
            amount_in_base_currency=Decimal(str(row[8])) if row_len > 8 and row[8] is not None else Decimal('0'),  # للتوافق مع الكود القديم
            # Multi-Currency Support
