...
class NotificationChecker(QThread):
    ...
    def _table_exists(self, table_name: str) -> bool:
        ...
        try:
            conn = self.db_manager.create_thread_connection()
            cur = conn.cursor()
            cur.execute(
    ...
    def check_payment_due(self) -> List[Notification]:
        ...
        try:
            conn = self.db_manager.create_thread_connection()
            cursor = conn.cursor()
            # نتأكد أن الجداول موجودة والأعمدة الضرورية متوفرة
            if not (
                self._table_exists("sales")
                and self._table_has_columns(
                    "sales",
                    ["id", "invoice_number", "total_amount", "due_date", "paid_amount"],
                )
            ):
                return notifications
            ...
            # الفواتير (المبيعات الآجلة) غير المسددة كلياً والمستحقة خلال 7 أيام
            cursor.execute(
                """
                SELECT s.id, s.invoice_number, COALESCE(c.name, 'زبون نقدي') as customer_name,
                       s.total_amount, s.due_date, s.paid_amount
                FROM sales s
                ...
                AND s.status NOT IN ('ملغية','مرتجعة')
                AND s.is_deleted = 0
                ORDER BY s.due_date
                LIMIT 5
            """,
    ...
    def check_reminders(self) -> List[Notification]:
        ...
        try:
            ...
            if not (
                self._table_exists("reminders")
                and self._table_has_columns(
                    "reminders",
                    ["id", "title", "description", "reminder_time", "status"],
                )
            ):
                return notifications
    ...
    def check_workflow_notifications(self) -> List[Notification]:
        ...
        try:
            ...
            if not (
                self._table_exists("workflow_instances")
                and self._table_exists("workflow_approvals")
                and self._table_exists("workflow_steps")
            ):
                return notifications
            ...
            for row in cursor.fetchall():
                ...
                notification = Notification(
                    ...
                    timestamp=datetime.now(),
                    priority=(
                        2 if deadline and datetime.fromisoformat(deadline) < datetime.now() + timedelta(hours=24) else 1
                    ),
...
class SmartNotificationsManager:
    ...
    def start(self):
        ...
        try:
            ...
            logging.getLogger(__name__).warning("Ignored exception in notifications_manager.py")
    ...
    def add_notification(self, notification: Notification):
        ...
        if self.system_tray:
            ...
            self.system_tray.showMessage(notification.title, notification.message, icon_type, 5000)  # 5 ثوانٍ
