...
class CurrencyDialog(QDialog):
    ...
    def __init__(self, currency: Optional[Currency] = None, currency_manager: CurrencyManager = None, parent=None):
...
class ExchangeRateDialog(QDialog):
    ...
    def __init__(self, exchange_rate: Optional[ExchangeRate] = None, 
...
class CurrencyManagementWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.currencies_table.setHorizontalHeaderLabels([
            "المعرف", "الرمز", "الاسم", "الرمز (Symbol)", 
            "الأرقام العشرية", "أساسية", "نشط"
        ])
        ...
        self.rates_table.setHorizontalHeaderLabels([
            "المعرف", "من العملة", "إلى العملة", "سعر الصرف",
            "تاريخ السريان", "تاريخ الانتهاء", "المصدر", "نشط"
        ])
    ...
    def load_exchange_rates(self):
        ...
        try:
            ...
            for row, rate in enumerate(rates):
                ...
                self.rates_table.setItem(row, 4, QTableWidgetItem(
                    rate.effective_date.isoformat() if rate.effective_date else ""
                ))
                self.rates_table.setItem(row, 5, QTableWidgetItem(
                    rate.expiry_date.isoformat() if rate.expiry_date else ""
                ))
