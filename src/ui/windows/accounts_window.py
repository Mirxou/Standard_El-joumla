...
class AccountsWindow(QMainWindow):
    ...
    def update_receivables_table(self, receivables):
        ...
        for row, account in enumerate(receivables):
            ...
            self.receivables_table.setItem(
                row,
                2,
                QTableWidgetItem(f"{last_payment:.2f} ج.م" if last_payment else "لا توجد"),
            )
            ...
            self.receivables_table.setItem(
                row,
                4,
                QTableWidgetItem(f"{last_payment:.2f} ج.م" if last_payment else "0.00 ج.م"),
            )
            ...
            pay_btn.clicked.connect(
                lambda checked, customer_id=account.get("entity_id"): self.make_customer_payment(customer_id)
            )
    ...
    def update_payables_table(self, payables):
        ...
        for row, account in enumerate(payables):
            ...
            pay_btn = QPushButton("دفع")
            pay_btn.clicked.connect(
                lambda checked, supplier_id=account.get("entity_id"): self.make_supplier_payment(supplier_id)
            )
            actions_layout.addWidget(pay_btn)
        ...
        for row, schedule in enumerate(schedules):
            ...
            if schedule.get("status") == "pending":
                ...
                cancel_btn.clicked.connect(
                    lambda checked, schedule_id=schedule.get("id"): self.cancel_schedule(schedule_id)
                )
    ...
    def pay_schedule(self, schedule_id):
        ...
        try:
            ...
            # فتح نافذة الدفع
            payment_type = (
                PaymentType.CUSTOMER_PAYMENT if schedule["entity_type"] == "customer" else PaymentType.SUPPLIER_PAYMENT
            )
            dialog = PaymentDialog(
                self.db_manager,
                parent=self,
