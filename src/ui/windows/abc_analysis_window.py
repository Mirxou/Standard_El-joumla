...
class ABCAnalysisWindow(QMainWindow):
    """نافذة تحليل ABC"""
    ...
    def create_results_tab(self) -> QWidget:
        ...
        self.results_table.setHorizontalHeaderLabels(
            [
                ...
                "كود المنتج",
                "اسم المنتج",
                "المبيعات السنوية (كمية)",
                "المبيعات السنوية (قيمة)",
                "السعر المتوسط",
                "المخزون الحالي",
                "قيمة المخزون",
                "الفئة",
    ...
    def create_summary_tab(self) -> QWidget:
        ...
        if isinstance(self.pie_chart_view, QWidget):
            try:
        ...
        if isinstance(self.bar_chart_view, QWidget):
            try:
    ...
    def draw_charts(self, summary: Dict):
        ...
        pie_series.append(
            f"فئة A ({summary.get('category_a_count', 0)})",
            summary.get("category_a_count", 0),
        )
        pie_series.append(
            f"فئة B ({summary.get('category_b_count', 0)})",
            summary.get("category_b_count", 0),
        )
        pie_series.append(
            f"فئة C ({summary.get('category_c_count', 0)})",
