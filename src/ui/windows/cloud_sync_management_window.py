...
from src.core.sync_engine import SyncDirection
...
class CloudSyncManagementWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.settings_table.setHorizontalHeaderLabels([
            "ID", "الاسم", "المزود", "المزامنة", "النسخ الاحتياطي", "التشفير", "آخر مزامنة"
        ])
        ...
        self.logs_table.setHorizontalHeaderLabels([
            "ID", "النوع", "الكيان", "الحالة", "الاتجاه", "التعرض", "التاريخ"
        ])
        ...
        self.conflicts_table.setHorizontalHeaderLabels([
            "ID", "الكيان", "الحالة", "الاستراتيجية", "تم الحل", "التاريخ"
        ])
    ...
    def load_sync_logs(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(rows):
                ...
                self.logs_table.setItem(row_idx, 2, QTableWidgetItem(f"{entity_type}:{entity_id}" if entity_id else ""))
    ...
    def load_conflicts(self):
        ...
        try:
            ...
            for row_idx, row in enumerate(rows):
                ...
                self.conflicts_table.setItem(row_idx, 3, QTableWidgetItem(row.get("resolution_strategy", "") or ""))
    ...
    def resolve_conflict(self, index):
        """حل تعارض"""
        row = index.row()
        conflict_id = int(self.conflicts_table.item(row, 0).text())
        
        QMessageBox.information(
            self, "قريباً",
            "ميزة حل التعارضات ستكون متاحة قريباً"
        )
...
class CloudSyncSettingsDialog(QDialog):
    ...
    def __init__(self, parent, cloud_sync_service: CloudSyncService, settings: Optional[CloudSyncSettings] = None):
        super().__init__(parent)
        self.cloud_sync_service = cloud_sync_service
