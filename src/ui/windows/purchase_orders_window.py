...
class PurchaseOrdersWindow(QMainWindow):
        ...
        try:
            ...
            for po in pos:
                ...
                priority_item = QTableWidgetItem(
                    po.priority.value if hasattr(po.priority, "value") else str(po.priority)
                )
    ...
    def _filter_pos(self):
        ...
        try:
            pos = self.po_service.get_all_purchase_orders(status=status, priority=priority, limit=500)
    ...
    def _display_po_details(self, po: PurchaseOrder):
        ...
        # الشروط
        self.delivery_terms_label.setText(
            po.delivery_terms.value if hasattr(po.delivery_terms, "value") else str(po.delivery_terms)
        )
        self.payment_terms_label.setText(
            po.payment_terms.value if hasattr(po.payment_terms, "value") else str(po.payment_terms)
        )
    ...
    def _new_purchase_order(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            if po:
                try:
                    po_id = self.po_service.create_purchase_order(po)  # noqa: F841
    ...
    def _submit_for_approval(self):
        ...
        try:
            if self.po_service.submit_for_approval(
                self.current_po.id, submitted_by=1
            ):  # TODO: الحصول على ID المستخدم الحالي
            ...
            if receiving_note:
                try:
                    ...
                    QMessageBox.information(
                        self,
                        "نجح",
                        f"تم استلام الشحنة: {receiving_note.receiving_number}",
                    )
                    self._load_purchase_orders()
                except Exception as e:
