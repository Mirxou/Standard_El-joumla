    ...
    def load_report(self):
        ...
        try:
            if report_type == "ملخص الأمان":
                ...
                result = self.security_reports_service.generate_threats_report(start_date=start_date, end_date=end_date)
        ...
        try:
            ...
            if file_path:
                if file_path.endswith(".json"):
                    ...
                    data_list = (
                        result.get("data", {}).get("events", [])
                        or result.get("data", {}).get("threats", [])
                        or result.get("data", {}).get("blocked_ips", [])
                    )
                    if data_list:
