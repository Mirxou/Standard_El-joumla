...
from PySide6.QtWidgets import (
    ...
    QTextEdit, QDoubleSpinBox, QDateEdit, QSplitter, QToolBar,
...
from src.models.warehouse import Warehouse, WarehouseTransfer
...
class WarehouseTransferWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.transfers_table.setHorizontalHeaderLabels([
            "رقم التحويل", "من", "إلى", "المنتج", "الكمية", "الحالة", "التاريخ", "ملاحظات"
        ])
    ...
    def create_transfer(self, *args, **kwargs):
        ...
        if dialog.exec() == QDialog.Accepted:
            try:
                ...
                if transfer_id:
                    QMessageBox.information(self, "نجح", f"تم إنشاء التحويل بنجاح\nرقم التحويل: {transfer.transfer_number}")
                    self.load_transfers()
                else:
    ...
    def complete_transfer(self):
        ...
        if transfer.status not in ["pending", "in_transit"]:
            QMessageBox.warning(self, "تحذير", f"لا يمكن إكمال تحويل في حالة '{self.get_status_text(transfer.status)}'")
            return
        
        reply = QMessageBox.question(
            self, "تأكيد الإكمال",
            f"هل أنت متأكد من إكمال التحويل '{transfer.transfer_number}'؟\n"
            f"سيتم نقل {transfer.quantity} وحدة من '{transfer.from_warehouse_name}' إلى '{transfer.to_warehouse_name}'.",
            QMessageBox.Yes | QMessageBox.No
        )
        
        if reply == QMessageBox.Yes:
            try:
                received_by = getattr(self, 'current_user_id', getattr(self.parent(), 'current_user_id', 1)) if self.parent() else 1
