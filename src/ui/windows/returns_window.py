...
class ReturnsWindow(QMainWindow):
    ...
    def _create_toolbar(self):
        ...
        self.new_return_btn.setStyleSheet(
            "background-color: #FF5722; color: white; padding: 8px 16px; font-weight: bold;"
        )
    ...
    def _load_returns(self):
        ...
        try:
            ...
            # تحميل المرتجعات
            returns = self.return_service.get_all_returns(return_type=return_type, status=status)
    ...
    def _display_return_details(self, ret: ReturnInvoice):
        ...
        for item in ret.items:
            ...
            self.items_table.setItem(
                row,
                4,
                QTableWidgetItem(item.return_reason.value if item.return_reason else "-"),
            )
    ...
    def _update_statistics(self):
        ...
        try:
            ...
            # Convert to float or handle MagicMocks gracefully
            try:
                total_value_str = f"{float(total_value):,.2f}" if not hasattr(total_value, '__dict__') else "0.00"
            except (ValueError, TypeError):
                total_value_str = "0.00"

            try:
                total_refunded_str = f"{float(total_refunded):,.2f}" if not hasattr(total_refunded, '__dict__') else "0.00"
            except (ValueError, TypeError):
                total_refunded_str = "0.00"

            self.total_returns_label.setText(f"إجمالي المرتجعات: {total_count}")
            self.total_value_label.setText(f"القيمة الإجمالية: {total_value_str} دج")
            self.total_refunded_label.setText(f"المبلغ المسترد: {total_refunded_str} دج")

        except Exception as e:
            self.logger.warning(f"خطأ في تحديث الإحصائيات: {e}", exc_info=True)
    ...
    def _approve_return(self):
        ...
        approved_by = (
            getattr(self.parent(), "current_user_id", getattr(self.parent(), "user_id", 1)) if self.parent() else 1
        )
    ...
    def _generate_credit_note(self):
        ...
        if credit_note_number:
            self._load_returns()
            self._on_return_selected()
            QMessageBox.information(self, "نجاح", f"تم إنشاء إشعار دائن بنجاح\nالرقم: {credit_note_number}")
        else:
