...
class ComplianceManagementWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.rules_table.setHorizontalHeaderLabels(["ID", "الاسم", "النوع", "الخطورة", "نشط", "تاريخ الإنشاء"])
        ...
        self.checks_table.setHorizontalHeaderLabels(["ID", "القاعدة", "التاريخ", "الحالة", "النتيجة"])
    ...
    def run_check(self):
        ...
        if check:
            ...
            QMessageBox.information(self, "اكتمل الفحص", f"تم تشغيل الفحص بنجاح!\n\nالحالة: {status_text}")
            self.load_checks()
        else:
