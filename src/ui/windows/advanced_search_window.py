...
from PySide6.QtWidgets import (
    ...
    QGroupBox, QCheckBox, QSpinBox, QDateEdit, QSplitter, QListWidget,
    QListWidgetItem, QMessageBox, QDialog, QFormLayout, QDialogButtonBox,
    QFrame, QMenu, QInputDialog, QFileDialog
)
...
class AdvancedSearchWindow(QMainWindow):
    ...
    def _display_joumla_results(self, result_dict: dict):
        ...
        columns = [
            "المعرف", "المنتج", "SKU", "باركود", 
            "سعر الجملة", "التكلفة", "الهامش", "نسبة الربح %", 
            "المخزون", "قيمة المخزون", "المؤشر"
        ]
    ...
    def _export_results(self):
        ...
        file_name, _ = QFileDialog.getSaveFileName(
            self, "تصدير النتائج", "", "CSV Files (*.csv);;All Files (*)"
        )
    ...
    def _print_current_quote(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(
            self, "حفظ ملف PDF", "quote.pdf", "PDF Files (*.pdf)"
        )
    ...
    def _add_to_quote(self, row_idx: int):
        ...
        try:
            ...
            if ok:
                data = {
                    'id': p_id, 'name': p_name, 
                    'wholesale_price': w_price, 'cost_price': c_price
                }
