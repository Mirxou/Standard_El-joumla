...
class PhysicalCountsWindow(QMainWindow):
    ...
    def load_data(self):
        ...
        try:
            ...
            # تحميل الجرود
            counts = self.service.get_all_counts(status=status, from_date=from_date_val, to_date=to_date_val)
            ...
            if search_text:
                counts = [
                    c
                    for c in counts
                    if search_text in c.count_number.lower() or search_text in (c.description or "").lower()
                ]
    ...
    def populate_table(self, counts: List[PhysicalCount]):
        ...
        for count in counts:
            ...
            # التاريخ
            date_str = (
                count.count_date.strftime("%Y-%m-%d") if isinstance(count.count_date, date) else str(count.count_date)
            )
    ...
    def start_count(self):
        ...
        reply = QMessageBox.question(self, "تأكيد", "هل تريد بدء هذا الجرد؟", QMessageBox.Yes | QMessageBox.No)
    ...
    def approve_count(self):
        ...
        reply = QMessageBox.question(self, "تأكيد الاعتماد", msg, QMessageBox.Yes | QMessageBox.No)
        ...
        if reply == QMessageBox.Yes:
            try:
                if self.service.approve_count(count_id, self.current_user_id, self.current_user_name):
                    self.load_data()
                    self.count_updated.emit()
                    QMessageBox.information(self, "نجاح", "تم اعتماد الجرد وإنشاء التسويات بنجاح")
                else:
