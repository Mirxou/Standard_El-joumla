...
class AccountingWindow(QMainWindow):
    ...
    def _load_accounts(self):
        ...
        for idx, account in enumerate(accounts):
            ...
            # الرمز
            self.accounts_table.setItem(idx, 0, QTableWidgetItem(account.account_code))
    ...
    def _load_journal_entries(self):
        ...
        try:
            ...
            for idx, entry in enumerate(entries):
                ...
                # رقم القيد
                self.journal_table.setItem(idx, 0, QTableWidgetItem(entry[1]))

                # التاريخ
                self.journal_table.setItem(idx, 1, QTableWidgetItem(str(entry[2])[:10]))

                # الوصف
                self.journal_table.setItem(idx, 2, QTableWidgetItem(entry[3] or ""))
