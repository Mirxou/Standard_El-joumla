...
class WebhookManagementWindow(QMainWindow):
    ...
    def load_webhook_logs(self):
        ...
        try:
            ...
            logs = self.webhook_service.get_webhook_logs(event_type=event_type, is_success=is_success, limit=100)
            ...
            for row, log in enumerate(logs):
                ...
                self.logs_table.setItem(
                    row,
                    3,
                    QTableWidgetItem(str(log.entity_id) if log.entity_id else "-"),
                )
                ...
                self.logs_table.setItem(
                    row,
                    5,
                    QTableWidgetItem(str(log.response_status) if log.response_status else "-"),
                )
                ...
                for col in range(self.logs_table.columnCount()):
                    ...
                    if item:
                        item.setToolTip("انقر نقراً مزدوجاً لعرض Payload")
        ...
        try:
            ...
            if webhook.headers:
                try:
                    ...
                    logging.getLogger(__name__).warning("Ignored exception in webhook_management_window.py")
    ...
    def retry_failed_webhooks(self):
        ...
        failed_logs = self.webhook_service.get_webhook_logs(is_success=False, limit=100)
    ...
    def view_payload(self, index):
        ...
        log = next((l for l in logs if l.id == log_id), None)  # noqa: E741
