...
from PySide6.QtWidgets import (
    ...
    QSpinBox, QPlainTextEdit, QFileDialog, QInputDialog
)
...
class EDIManagementWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        self.partners_table.setHorizontalHeaderLabels([
            "ID", "الاسم", "الكود", "النوع", "المعيار", "الاتصال", "نشط", "معالجة تلقائية"
        ])
        ...
        self.documents_table.setHorizontalHeaderLabels([
            "ID", "النوع", "الرقم", "الشريك", "الاتجاه", "الحالة", "تاريخ الإنشاء", "الأخطاء"
        ])
    ...
    def load_partners(self):
        ...
        try:
            ...
            for row, partner in enumerate(partners):
                ...
                self.partners_table.setItem(row, 4, QTableWidgetItem(f"{partner.edi_standard} {partner.edi_version}"))
    ...
    def load_documents(self):
        ...
        try:
            ...
            documents = self.edi_service.get_all_documents(
    ...
    def parse_edi_file(self):
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            self, "اختر ملف EDI", "", "EDI Files (*.edi *.txt);;All Files (*)"
        )
        ...
        try:
            ...
            # اختيار المعيار
            standard, ok = QInputDialog.getItem(
                self, "اختر المعيار", "المعيار:", ["EDIFACT", "X12"], 0, False
            )
            ...
            if result.success:
                QMessageBox.information(
                    self, "نجاح",
                    f"تم تحليل المستند بنجاح!\nالنوع: {result.document_type}\nالبيانات: {json.dumps(result.data, indent=2, ensure_ascii=False)}"
                )
            else:
    ...
    def generate_edi(self):
        ...
        QMessageBox.information(
            self, "قريباً",
            "ميزة توليد EDI ستكون متاحة قريباً"
        )
...
class EDIDocumentViewDialog(QDialog):
    ...
    def setup_ui(self):
        ...
        if self.document.created_at:
            info_layout.addRow("تاريخ الإنشاء:", QLabel(self.document.created_at.strftime("%Y-%m-%d %H:%M:%S")))
