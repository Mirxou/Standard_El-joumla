    ...
    def data(self, index: QModelIndex, role: int = Qt.DisplayRole):
            ...
            if col == 6:  # عمود الحالة
                try:
                    ...
                    if 'ملغية' in status or 'cancelled' in status:
                        ...
                        return QColor("#2c3e50")  # رمادي - أخرى
                except (IndexError, KeyError, ValueError, TypeError):
                    return QColor("#2c3e50")
            elif col == 5:  # المبلغ المتبقي
                try:
                    remaining = float(self._data.iloc[row]['remaining_amount']) if 'remaining_amount' in self._data.columns and pd.notna(self._data.iloc[row]['remaining_amount']) else 0
                    if remaining > 0:
        ...
        # UserRole - بيانات إضافية (sale_id)
            ...
            if col == 0:  # في عمود رقم الفاتورة نخزن sale_id
                try:
                    return int(self._data.iloc[row]['id']) if 'id' in self._data.columns and pd.notna(self._data.iloc[row]['id']) else None
