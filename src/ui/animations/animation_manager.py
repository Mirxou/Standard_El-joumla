...
from PySide6.QtCore import QObject, QPropertyAnimation, QEasingCurve, QParallelAnimationGroup, QSequentialAnimationGroup, Property, Signal
...
class AnimationManager(QObject):
    ...
    def fade_out(self, widget: QWidget, duration: int = 300, easing: QEasingCurve.Type = QEasingCurve.InCubic) -> str:
        """
        حركة fade out
    ...
    def slide_in(self, widget: QWidget, direction: str = "right", duration: int = 300, easing: QEasingCurve.Type = QEasingCurve.OutCubic) -> str:
        """
        حركة slide in
    ...
    def float_animation(self, widget: QWidget, amplitude: int = 10, duration: int = 2000, easing: QEasingCurve.Type = QEasingCurve.InOutSine) -> str:
        """
        حركة float (طفو)
        ...
        animation.setEndValue(original_pos + widget.mapFromParent(widget.rect().topLeft() + widget.rect().bottomLeft()) * amplitude / 100)
        animation.setEasingCurve(easing)
    ...
    def scale_animation(self, widget: QWidget, start_scale: float = 0.8, end_scale: float = 1.0, duration: int = 300, easing: QEasingCurve.Type = QEasingCurve.OutBack) -> str:
        """
