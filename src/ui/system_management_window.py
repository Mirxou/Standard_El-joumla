...
class SystemManagementWindow(QDialog):
    ...
    def create_backup_tab(self):
        ...
        self.backups_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        self.backups_table.horizontalHeader().setMinimumSectionSize(120)
        self.backups_table.horizontalHeader().setDefaultSectionSize(150)
    ...
    def cleanup_backups(self):
        ...
        if reply == QMessageBox.Yes:
            self.backup_thread = BackupThread(self.backup_service, "cleanup", keep_count=keep_count)
            self.backup_thread.finished.connect(self.on_backup_finished)
            self.backup_thread.start()
    ...
    def on_restore_finished(self, success, message):
        ...
        if success:
            self.notify.show_success("نجح", f"{message}\n\nيُنصح بإعادة تشغيل التطبيق.")
            self.load_backups()
        else:
            ...
            for row, backup in enumerate(backups):
                ...
                if isinstance(date_str, str) and date_str != "-":
                    try:
                        ...
                        logging.getLogger(__name__).warning("Ignored exception in system_management_window.py")
    ...
    def cleanup_data(self):
        ...
        if ok:
            try:
                days_int = int(days)
                self.perf_thread = PerformanceThread(self.performance_service, "cleanup", days=days_int)
                self.perf_thread.finished.connect(self.on_performance_finished)
                self.perf_thread.start()
            except ValueError:
    ...
    def analyze_performance(self):
        """تحليل الأداء"""
        try:
            res = self.performance_service.analyze_performance()
            ...
            result_text += f"حجم قاعدة البيانات: {analysis.get('database_size_mb', 0):.2f} MB\n"
            result_text += f"عدد الجداول: {analysis.get('total_tables', 0)}\n"
            result_text += f"عدد الفهارس: {analysis.get('total_indexes', 0)}\n"
            result_text += f"صفحات فارغة: {analysis.get('freelist_pages', 0)}\n"

            frag = analysis.get("fragmentation_percent", 0)
            if frag > 0:
                result_text += f"نسبة التجزئة: {frag:.2f}%\n"

            result_text += "\n=== أكبر الجداول ===\n"
            for table_data in analysis.get("largest_tables", []):
                result_text += f"{table_data.get('table', '-')}: {table_data.get('records', 0):,} صف\n"

            result_text += "\n=== التوصيات ===\n"
            if frag > 10:
                result_text += "• يُنصح بتشغيل VACUUM لتقليل التجزئة\n"
            if analysis.get("freelist_pages", 0) > 100:
                result_text += "• يوجد صفحات فارغة كثيرة - قم بتشغيل التحسين\n"
            if analysis.get("database_size_mb", 0) > 100:
                result_text += "• قاعدة البيانات كبيرة - فكر في أرشفة البيانات القديمة\n"

            if not any(
                [
                    frag > 10,
                    analysis.get("freelist_pages", 0) > 100,
                    analysis.get("database_size_mb", 0) > 100,
                ]
            ):
    ...
    def load_performance_info(self):
        ...
        try:
            # Database metrics
            res = self.performance_service.analyze_performance()
            analysis = res.get("analysis", {}) if res.get("success") else {}

            self.perf_labels["db_size"].setText(f"{analysis.get('database_size_mb', 0):.2f} MB")
            self.perf_labels["table_count"].setText(str(analysis.get("total_tables", 0)))
            self.perf_labels["index_count"].setText(str(analysis.get("total_indexes", 0)))
            self.perf_labels["free_pages"].setText(str(analysis.get("freelist_pages", 0)))

            # System info
            res_system = self.performance_service.get_system_info()
            system_info = res_system.get("system", {}) if res_system.get("success") else {}

            ram_percent = system_info.get("ram_percent", 0)
    ...
    def create_maintenance_tab(self):
        ...
        self.maintenance_table.horizontalHeader().setSectionResizeMode(QHeaderView.Interactive)
        self.maintenance_table.horizontalHeader().setMinimumSectionSize(120)
        self.maintenance_table.horizontalHeader().setDefaultSectionSize(150)
