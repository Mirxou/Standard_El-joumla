...
class WebSocketClientRunnable(BaseRunnable):
    ...
    def __init__(
        ...
        signals: Optional[WebSocketSignals] = None,
                ...
                async with websockets.connect(url) as websocket:
                    ...
                    while self.should_connect and self.is_connected:
                        try:
                            ...
                            message = await asyncio.wait_for(websocket.recv(), timeout=30.0)  # 30 ثانية
                            ...
                            logger.log(logging.ERROR, f"خطأ في استقبال رسالة: {e}")
                            break
    ...
    async def _run_websocket(self):
        ...
        while self.should_connect:
            try:
                ...
                logger.log(logging.ERROR, f"❌ خطأ في WebSocket connection: {e}")
                self.is_connected = False
        ...
        if self.worker:
            ...
            if hasattr(self.worker, "wait"):
                try:
                    ...
                    logging.getLogger(__name__).warning("Ignored exception in websocket_client.py")
            self.worker = None
            self.signals = None
    ...
    def _handle_message(self, message: Dict[str, Any]):
        ...
        try:
            ...
            if message_type == "data_update":
                ...
                logger.log(logging.ERROR, f"WebSocket error: {data.get('message', 'Unknown error')}")
                self._emit_event("error", data)

        except Exception as e:
            logger.log(logging.ERROR, f"خطأ في معالجة رسالة WebSocket: {e}")
        ...
        if event in self.event_handlers:
            for handler in self.event_handlers[event]:
                try:
                    ...
                    logger.log(logging.ERROR, f"خطأ في event handler: {e}")
