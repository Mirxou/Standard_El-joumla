...
"""
                for i, (name, balance) in enumerate(top_balance, 1):
                    report += f"  {i}. {name:<30} {float(balance):>15,.2f} ريال\n"
            else:
                report += """💰 العملاء الأعلى رصيداً مستحقاً:
─────────────────────────────────────────────────────────────────────────────
...
"""
            ...
            if top_buyers:
                ...
                for i, (name, count, amount) in enumerate(top_buyers, 1):
                    ...
                    report += f"  {i}. {name:<30} {count:>5} فاتورة  {amount_val:>15,.2f} ريال\n"
            else:
                report += f"\n🛍️  العملاء الأكثر شراءً:\n"
                report += "─────────────────────────────────────────────────────────────────────────────\n"
...
"""
            ...
            if top_suppliers:
                ...
                for i, (name, count, amount) in enumerate(top_suppliers, 1):
                    amount_val = float(amount) if amount else 0
                    report += f"  {i}. {name:<30} {count:>5} عملية   {amount_val:>15,.2f} ريال\n"
            else:
                report += f"📦 الموردين الأكثر عمليات:\n"
                report += "─────────────────────────────────────────────────────────────────────────────\n"
                report += "  لا توجد عمليات شراء مسجلة\n"
...
─────────────────────────────────────────────────────────────────────────────
  إجمالي الرصيد:           {float(customers_balance):>20,.2f} ريال
  متوسط الرصيد:           {float(customers_balance)/max(customers_total, 1):>20,.2f} ريال
...
"""
            ...
            if float(sales_total) > float(purchases_total):
                ...
                report += f"  ✓ صافي الأرباح المتوقع: {profit:,.2f} ريال\n"
