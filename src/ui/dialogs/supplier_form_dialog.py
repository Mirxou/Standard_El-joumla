...
class SupplierFormDialog(BaseDialog):
    ...
    def load_supplier(self):
        ...
        try:
            ...
            self.notify.show_error(
                ...
                f"{self.i18n.get_message('supplier_load_failed')}: {str(e)}",
            )
    ...
    def save_supplier(self):
        ...
        if not name:
            self.notify.show_warning(
                ...
                self.i18n.get_message("supplier_name_required"),
            )
            return
        ...
        try:
            if self.supplier_id:
                ...
                self.notify.show_success(
                    ...
                    self.i18n.get_message("supplier_updated"),
                )
            else:
                # إضافة جديد
                query = """
                ...
                self.notify.show_success(
                    self.i18n.get_message("success"),
                    self.i18n.get_message("supplier_added"),
                )
            ...
            self.notify.show_error(
                self.i18n.get_message("error"),
                f"{self.i18n.get_message('supplier_save_failed')}: {str(e)}",
            )
