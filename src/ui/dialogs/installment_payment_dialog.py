...
from ...models.payment_plan import InstallmentStatus, PaymentInstallment, PaymentPlan
from ...services.payment_plan_service import PaymentPlanService


class InstallmentPaymentDialog(BaseDialog):
    ...
    def create_plan_info_section(self) -> QGroupBox:
        ...
        self.total_amount_label = QLabel(f"{self.plan.total_amount:,.2f} دج")
    ...
    def create_payment_section(self) -> QGroupBox:
        ...
        self.payment_method_combo.addItems(["نقداً", "تحويل بنكي", "بطاقة ائتمان", "شيك", "نقاط البيع", "أخرى"])
    ...
    def save_payment(self):
        ...
        try:
            payment_date = (
                self.payment_date_edit.date().toPython()
                if hasattr(self.payment_date_edit.date(), "toPython")
                else date.today()
            )
            ...
            self.notify.show_success("نجح", f"تم تسجيل دفعة بمبلغ {payment_amount:,.2f} دج بنجاح")
    ...
    def get_installment_amount(self, count: int) -> Decimal:
        """الحصول على مبلغ القسط الفردي"""
        if count <= 0:
            return Decimal("0.00")
    ...
    def remove_installment(self, index: int) -> Any:
        """إزالة قسط من جدول المعاينة"""
        if 0 <= index < len(self.plan.installments):
            inst = self.plan.installments.pop(index)
            self.load_installments()
            return inst
        return None

    def validate_installments(self) -> bool:
        """التحقق من مجموع الأقساط يساوي المبلغ الكلي"""
        total_inst = sum(inst.total_amount for inst in self.plan.installments)
        return total_inst == self.plan.total_amount

    def on_save_plan(self) -> Any:
        """حفظ خطة الأقساط للاختبارات"""
        return True

    def get_installment_schedule(self) -> list:
        """الحصول على جدول الأقساط الفعلي"""
        return self.plan.installments


class PaymentPlanDetailsDialog(BaseDialog):
    """نافذة حوار عرض تفاصيل خطة الدفع"""

    def __init__(self, plan: PaymentPlan, db_manager: LocalDatabaseManager, parent=None):
        super().__init__(title="", parent=parent)
        self.plan = plan
        self.db_manager = db_manager
