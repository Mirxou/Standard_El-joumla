...
from PySide6.QtGui import QCloseEvent, QFont, QPixmap, QShowEvent
...
try:
    ...
    from ...services.product_service_enhanced import (
        ProductService as EnhancedProductService,
    )
except Exception:
        ...
        try:
            ...
            if barcode:
                ...
                if existing_product and (not self.product_id or existing_product.id != self.product_id):
                    self.validation_completed.emit(
                        False,
                        f"الباركود موجود مسبقاً للمنتج: {existing_product.name}",
                        {},
                    )
                    return
            ...
            for product in existing_products:
                if product.name.lower() == name.lower() and (not self.product_id or product.id != self.product_id):
                    self.validation_completed.emit(False, f"اسم المنتج موجود مسبقاً", {})  # noqa: F541
                    return
            ...
            if self.product_data.get("track_stock", True):
                ...
                if max_stock > 0 and min_stock > max_stock:
                    self.validation_completed.emit(
                        False,
                        "الحد الأدنى للمخزون لا يمكن أن يكون أكبر من الحد الأقصى",
                        {},
                    )
                    return
...
class ProductDialog(BaseDialog):
    ...
    def setup_ui(self):
        ...
        _title = (
            self.i18n.get_message("product_edit_title")
            if self.is_edit_mode
            else self.i18n.get_message("product_new_title")
        )
        self.title_text = _title
    ...
    def create_basic_tab(self) -> QWidget:
        ...
        basic_group = QGroupBox(self.i18n.get_message("basic_info"))
        ...
        basic_layout.addRow(f"{self.i18n.get_message('table_product')} *:", self.name_edit)
        ...
        basic_layout.addRow(f"{self.i18n.get_message('barcode', default='الباركود')}:", barcode_layout)

        # الفئة
        self.category_combo = QComboBox()
        self.category_combo.setEditable(True)
        basic_layout.addRow(f"{self.i18n.get_message('filter', default='الفئة')}:", self.category_combo)

        # المورد
        self.supplier_combo = QComboBox()
        self.supplier_combo.setEditable(True)
        basic_layout.addRow(f"{self.i18n.get_message('suppliers')}:", self.supplier_combo)

        # الوحدة
        self.unit_edit = QLineEdit()
        self.unit_edit.setPlaceholderText(self.i18n.get_message("enter_unit"))
        basic_layout.addRow(f"{self.i18n.get_message('table_unit')}:", self.unit_edit)

        layout.addWidget(basic_group)

        # مجموعة الوصف
        desc_group = QGroupBox(self.i18n.get_message("description_notes"))
    ...
    def create_pricing_tab(self) -> QWidget:
        ...
        # مجموعة الأسعار
        pricing_group = QGroupBox(self.i18n.get_message("pricing"))
        ...
        self.cost_price_spin.setSuffix(f" {self.i18n.get_message('currency_symbol')}")
        pricing_layout.addRow(f"{self.i18n.get_message('cost_price_label')}:", self.cost_price_spin)
        ...
        self.selling_price_spin.setSuffix(f" {self.i18n.get_message('currency_symbol')}")
        pricing_layout.addRow(
            f"{self.i18n.get_message('selling_price_label')} *:",
        ...
        # مجموعة المخزون
        stock_group = QGroupBox(self.i18n.get_message("stock"))
        ...
        stock_layout.addRow(f"{self.i18n.get_message('current_stock')}:", self.stock_quantity_spin)

        # الحد الأدنى للمخزون
        self.min_stock_spin = QSpinBox()
        self.min_stock_spin.setRange(0, 999999)
        stock_layout.addRow(
            f"{self.i18n.get_message('table_stock')} ({self.i18n.get_message('stock_low')}):",
            self.min_stock_spin,
        )
        ...
        stock_layout.addRow(
            f"{self.i18n.get_message('table_stock')} ({self.i18n.get_message('stock_good', default='الأقصى')}):",
            self.max_stock_spin,
        )
        ...
        stock_layout.addRow(
            f"{self.i18n.get_message('location', default='موقع المخزون')}:",
            self.location_edit,
        )
    ...
    def create_details_tab(self) -> QWidget:
        ...
        status_group = QGroupBox(self.i18n.get_message("status_settings"))
        status_layout = QFormLayout(status_group)
        status_layout.setSpacing(10)

        # حالة النشاط
        self.is_active_check = QCheckBox(self.i18n.get_message("product_active"))
        self.is_active_check.setChecked(True)
        status_layout.addRow("", self.is_active_check)

        # قابل للبيع
        self.is_sellable_check = QCheckBox(self.i18n.get_message("sellable"))
        self.is_sellable_check.setChecked(True)
        status_layout.addRow("", self.is_sellable_check)

        # قابل للشراء
        self.is_purchasable_check = QCheckBox(self.i18n.get_message("purchasable"))
        self.is_purchasable_check.setChecked(True)
        status_layout.addRow("", self.is_purchasable_check)

        # تتبع المخزون
        self.track_stock_check = QCheckBox(self.i18n.get_message("track_stock"))
        ...
        extra_group = QGroupBox(self.i18n.get_message("additional_info"))
    ...
    def create_history_tab(self) -> QWidget:
        ...
        self.history_table.setHorizontalHeaderLabels(["التاريخ", "النوع", "الكمية", "السبب", "المستخدم"])
    ...
    def load_data(self):
        ...
        try:
            ...
            self.category_combo.addItem(f"-- {self.i18n.get_message('filter', default='اختر الفئة')} --", None)
            ...
            self.supplier_combo.addItem(
                f"-- {self.i18n.get_message('suppliers', default='اختر المورد')} --",
                None,
            )
            ...
            QMessageBox.critical(
                ...
                f"{self.i18n.get_message('load_failed')}: {str(e)}",
            )
    ...
    def generate_barcode(self):
        ...
        try:
            ...
            QMessageBox.warning(
                ...
                self.i18n.get_message("barcode_generation_failed"),
            )
        ...
        try:
            ...
            if cost_price > 0 and selling_price > 0:
                ...
                self.profit_margin_label.setText(
                    self.i18n.get_message("profit_margin_label", percent=f"{profit_margin:.2f}")
                )
    ...
    def collect_form_data(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "stock_quantity": (
                self.stock_quantity_spin.value()
                if hasattr(self, "stock_quantity_spin") and self.track_stock_check.isChecked()
                else 0
            ),
            "min_stock_level": (
                self.min_stock_spin.value()
                if hasattr(self, "min_stock_spin") and self.track_stock_check.isChecked()
                else 0
            ),
            "max_stock_level": (
                self.max_stock_spin.value()
                if hasattr(self, "max_stock_spin") and self.track_stock_check.isChecked()
                else 0
            ),
            ...
            "is_purchasable": getattr(self, "is_purchasable_check", None)
            and self.is_purchasable_check.isChecked()
            or True,
            ...
            "expiry_date": getattr(self, "expiry_date_edit", None)
            and self.expiry_date_edit.date().toString(Qt.ISODate)
            or None,
        ...
        try:
            ...
            if not product_name:
                QMessageBox.warning(
                    self,
                    self.i18n.get_message("error"),
                    self.i18n.get_message("product_name_required"),
                )
                return
            ...
            try:
                ...
                if cost_price < 0:
                    QMessageBox.warning(
                        self,
                        self.i18n.get_message("error"),
                        self.i18n.get_message("cost_price_negative"),
                    )
                    return
                ...
                if selling_price <= 0:
                    QMessageBox.warning(
                        self,
                        self.i18n.get_message("error"),
                        self.i18n.get_message("selling_price_zero"),
                    )
                    return
                ...
                QMessageBox.warning(
                    self,
                    self.i18n.get_message("error"),
                    f"{self.i18n.get_message('invalid_prices')}: {str(e)}",
                )
                return
            ...
            try:
                ...
                if stock_quantity < 0:
                    QMessageBox.warning(
                        self,
                        self.i18n.get_message("error"),
                        self.i18n.get_message("quantity_negative"),
                    )
                    return
            except (ValueError, TypeError) as e:
                QMessageBox.warning(
                    self,
                    self.i18n.get_message("error"),
                    f"{self.i18n.get_message('invalid_quantity')}: {str(e)}",
                )
                return
            ...
            self.validation_worker = ProductValidationWorker(self.product_manager, product_data, product_id)
            self.validation_worker.validation_completed.connect(
                lambda success, message, data: self.on_validation_completed(success, message, data, close_after_save)
            )
                    ...
                    if success:
                        ...
                        if updated_product:
                            ...
                            try:
                                ...
                                if self.logger:
                                    self.logger.debug("✅ تم إطلاق إشارات: inventory_updated, inventory_item_updated")
                            except Exception as e:
                                if self.logger:
                            ...
                            QMessageBox.information(
                                ...
                                self.i18n.get_message("product_updated"),
                            )
                            if close_after_save:
                                self.accept()
                        else:
                            QMessageBox.critical(
                                self,
                                self.i18n.get_message("error"),
                                self.i18n.get_message("load_failed"),
                            )
                    else:
                        QMessageBox.critical(
                            self,
                            self.i18n.get_message("error"),
                            self.i18n.get_message("update_failed"),
                        )
                else:
                    # legacy update
                    ...
                    if product_id:
                        ...
                        if created_product:
                            ...
                            try:
                                ...
                                if self.logger:
                                    self.logger.debug("✅ تم إطلاق إشارات: inventory_updated, inventory_item_added")
                            except Exception as e:
                                if self.logger:
                                    self.logger.warning(f"⚠️ فشل إطلاق الإشارات: {e}")
    ...
    def search_similar_products(self):
        ...
        try:
            ...
            if similar_products:
                ...
                products_list = "\n".join([f"- {p.name} (الباركود: {p.barcode})" for p in similar_products[:5]])
    ...
    def _finalize_close(self):
        ...
        if hasattr(self, "_close_event"):
            self.close()  # Fixed: avoided deleted C++ object
