...
class PredictiveEngine:
    ...
    def forecast_sales(self, product_id: Optional[int] = None, days: int = 30) -> List[SalesForecast]:
        ...
        if product_id:
            ...
            try:
                products = self.db.execute_query("SELECT * FROM products WHERE id = ?", (product_id,))
        ...
        for product in products:
            ...
            recommended_reorder = max(predicted_sales * 1.2, reorder_point * 2)  # 20% احتياطي
    ...
    def analyze_customer_behavior(self, customer_id: Optional[int] = None) -> List[CustomerInsight]:
        ...
        if customer_id:
            try:
                customers = self.db.execute_query("SELECT * FROM customers WHERE id = ?", (customer_id,))
        ...
        for customer in customers:
            ...
            purchase_dates = [datetime.fromisoformat(p["date"]) for p in purchases if p.get("date")]
            purchase_dates.sort()
            ...
            # تصنيف العميل
            customer_segment = self._segment_customer(total_purchases, purchase_frequency, len(purchases))

            # حساب القيمة الدائمة للعميل (LTV)
            # تقدير بسيط: الإنفاق السنوي × 3 سنوات
            if len(purchase_dates) > 1:
                annual_spending = total_purchases * (365 / max((purchase_dates[-1] - purchase_dates[0]).days, 1))
            else:
    ...
    def get_product_recommendations(self, customer_id: int, limit: int = 5) -> List[Dict]:
        """
                ...
                insights.append(
                    {
                        ...
                        "message": f"⚠️ تنبيه ذكي: المنتج '{forecast.product_name}' سينفد خلال {forecast.days_until_stockout if forecast.days_until_stockout is not None else 0} أيام بناءً على نمط الشراء الحالي. يُنصح بطلب {round(forecast.recommended_reorder_quantity)} قطعة الآن.",  # noqa: E501
                    }
                )
        ...
        for customer in customer_insights:
            if customer.churn_risk > 0.8:  # احتمال عالي لفقدان العميل
                insights.append(
                    {
                        "type": "OPPORTUNITY",
                        "message": f"💡 فرصة: العميل '{customer.customer_name}' لم يشترِ منذ فترة (خطر الفقدان {int(customer.churn_risk*100)}%). النظام يقترح إرسال عرض خصم 10% على منتجاته المفضلة لاستعادته.",  # noqa: E501
                    }
                )
    ...
    def _segment_customer(self, total_purchases: float, frequency: float, order_count: int) -> str:
        """تصنيف العميل"""
