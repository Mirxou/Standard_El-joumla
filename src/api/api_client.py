...
class APIClient:
    ...
    def _make_request(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if self.enable_logging and attempt > 0:
                    self.logger.debug(f"إعادة محاولة {attempt}/{self.max_retries} للطلب: {method} {url}")
                ...
                if self._should_retry(response.status_code):
                    if attempt < self.max_retries:
                        ...
                        if self.enable_logging:
                            self.logger.error(f"فشل الطلب بعد {self.max_retries + 1} محاولات: "
                ...
                if self._should_retry(0, e) and attempt < self.max_retries:
                    ...
                    if self.enable_logging:
                        self.logger.warning(
                            f"خطأ في الاتصال: {str(e)} - " f"إعادة المحاولة بعد {backoff_time:.2f} ثانية"
                        )
                    time.sleep(backoff_time)
                    continue
                else:
                    if self.enable_logging:
                        self.logger.error(f"فشل الطلب بعد {attempt + 1} محاولات: {method} {url} - {str(e)}")
                    return None
        ...
        if self.enable_logging and last_exception:
            self.logger.error(f"فشل الطلب نهائياً بعد {self.max_retries + 1} محاولات: " f"{method} {url} - {str(last_exception)}"
            )
...
class HybridDataService:
    ...
    def sync_pending_changes(self) -> Dict[str, int]:
        ...
        with self.db.get_cursor() as cur:
            cur.execute("SELECT id, entity_type, entity_id, operation FROM sync_queue WHERE synced = 0")
            rows = cur.fetchall()
            columns = [desc[0] for desc in cur.description] if cur.description else []
                ...
                try:
                    ...
                    if entity_type == "product":
                        if operation == "create":
                            ...
                            product = self.db.execute_query("SELECT * FROM products WHERE id = ?", (entity_id,))
                            if product:
                                # إرسال إلى API
                        ...
                        if operation == "create":
                            ...
                            if sale:
                                result = self.api.put(
                                    f"sales/{entity_id}",
                                    sale[0] if isinstance(sale, list) else sale,
                                )
                                success = result is not None
                        elif operation == "delete":
