...
class SyncService:
    ...
    def _pull_delta(self, since: datetime) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.circuit_breaker.call(
                lambda: self.api_client.get("/api/v1/sync/delta", params={"last_synced": since.isoformat()})
            )
                ...
                try:
                    response = self.circuit_breaker.call(
                        lambda: self.api_client.post(
                            "/api/v1/sync/push",
                            data={"table_name": table_name, "items": items},
                        )
                    )
    ...
    def _check_conflict(self, table_name: str, remote_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        if local_updated and remote_updated:
            try:
                ...
                remote_dt = (
                    datetime.fromisoformat(remote_updated) if isinstance(remote_updated, str) else remote_updated
                )
