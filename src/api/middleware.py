...
class AuthMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, auth_manager: JWTAuthManager, exclude_paths: Optional[list] = None):
        """
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(self, app: ASGIApp, rate_limiter: APIRateLimiter, exclude_paths: Optional[list] = None):
        """
...
class LoggingMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable):
        ...
        try:
            ...
            self.logger.error(
                f"API Error: {method} {path} - {ip_address} - {str(e)} - {process_time:.3f}s"
            )
            raise
...
def setup_middleware(
    ...
    """
    # Logging Middleware (يجب أن يكون أولاً)
