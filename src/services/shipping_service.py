...
class ShippingService:
    ...
    def create_shipment(
        ...
        try:
            ...
            if provider == "FEDEX":
                return self._create_fedex_shipment(
                    integration,
                    sale_id,
                    origin_address,
                    destination_address,
                ...
                return self._create_dhl_shipment(
                    integration,
                    sale_id,
                    origin_address,
                    destination_address,
        ...
        try:
            ...
            shipment_data = {
                ...
                "requestedShipment": {
                    ...
                    "requestedPackageLineItems": [{"weight": {"units": "KG", "value": float(weight)}}],
                },
            }
            ...
            if shipment_response.status_code not in [200, 201]:
                return (
                    False,
                    None,
                    {"error": f"فشل إنشاء الشحنة: {shipment_response.text}"},
                )

            response_data = shipment_response.json()
            shipment_id = (
                response_data.get("output", {}).get("transactionShipments", [{}])[0].get("masterTrackingNumber", "")
            )
            ...
            self._save_shipment(
                ...
                status="PENDING",
                shipping_status=response_data.get("output", {})
                .get("transactionShipments", [{}])[0]
                .get("shipmentDocuments", [{}])[0]
                .get("contentType", ""),
    ...
    def _create_dhl_shipment(
        ...
        try:
            ...
            shipment_data = {
                "plannedShippingDateAndTime": {"shippingDate": datetime.now().strftime("%Y-%m-%d")},
                "pickup": {"isRequested": False},
                "productCode": "P",
                "accounts": [{"number": api_key}],
                "content": {
                    "packages": [
                        {
                            "weight": float(weight),
            ...
            response_data = shipment_response.json()
            shipment_id = response_data.get(
                "shipmentTrackingNumber",
                f"DHL_{sale_id}_{datetime.now().strftime('%Y%m%d%H%M%S')}",
            )
    ...
    def _save_shipment(
        ...
        try:
            ...
            values = (
                ...
                status,
                shipping_status,
                float(shipping_cost) if shipping_cost else None,
                currency,
                provider_response,
            )
