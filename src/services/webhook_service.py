...
from src.core.webhook_dispatcher import WebhookDeliveryResult, get_webhook_dispatcher
...
class WebhookService:
    ...
    def trigger_webhook(
        ...
        try:
            ...
            webhooks = self.get_all_webhooks(event_type=event_type, is_active=True, company_id=company_id)
                ...
                try:
                    ...
                    if webhook.headers:
                        try:
                            ...
                            logging.getLogger(__name__).warning("Ignored exception in webhook_service.py")
                    ...
                    if not success:
                        if self.logger:
                            self.logger.warning(
                                f"⚠️ تم تجاوز Rate Limit لـ Webhook {webhook.name} " f"(ID: {webhook.id})"
                            )

                    if self.logger:
                        self.logger.debug(
                            f"✅ تم إطلاق Webhook: {webhook.name} " f"(Event: {event_type}, Entity: {entity_id})"
                        )

                except Exception as e:
                    if self.logger:
                        self.logger.error(f"❌ خطأ في إطلاق Webhook {webhook.name}: {e}",
                            exc_info=True,
                        )
