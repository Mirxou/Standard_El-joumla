...
class CloudSyncService:
    ...
    def create_settings(self, settings: CloudSyncSettings) -> Optional[int]:
        ...
        try:
            ...
            values = (
                ...
                1 if settings.auto_sync else 0,
                settings.sync_interval_minutes,
                1 if settings.backup_enabled else 0,
                1 if settings.auto_backup else 0,
                settings.backup_interval_hours,
                settings.backup_time,
                1 if settings.encryption_enabled else 0,
    ...
    def _get_remote_entity(
        self, settings: CloudSyncSettings, entity_type: str, entity_id: int
    ...
    def _get_from_gcs(self, settings: CloudSyncSettings, entity_type: str, entity_id: int) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            client = storage.Client.from_service_account_json(settings.config if settings.config else None)
    ...
    def _log_sync(self, settings_id: int, result: SyncResult):
        ...
        try:
            ...
            values = (
                settings_id,
                "FULL_SYNC" if result.entity_type else "ENTITY_SYNC",
                result.entity_type,
                result.entity_id,
                result.status.value,
                (result.direction.value if isinstance(result.direction, SyncDirection) else str(result.direction)),
                result.local_hash,
                result.remote_hash,
                0,
