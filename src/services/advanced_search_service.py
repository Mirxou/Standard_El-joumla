...
class AdvancedSearchService:
    ...
    def get_suggestions(self, keyword: str, entity: SearchEntity, limit: int = 10) -> List[SearchSuggestion]:
        ...
        for row in results:
            suggestions.append(SearchSuggestion(text=row["text"], entity=entity, score=1.0))
    ...
    def save_filter(self, saved_filter: SavedFilter) -> int:
        """حفظ فلتر"""
        sql = """
            INSERT INTO saved_filters (name, description, entity, query_data, is_default, is_shared, created_by, created_at, updated_at)  # noqa: E501
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """
        now = datetime.now()
    ...
    def list_saved_filters(
        self, entity: Optional[SearchEntity] = None, user_id: Optional[int] = None
    ) -> List[SavedFilter]:
