...
class CRMService:
    ...
    def create_customer(self, customer_data: Dict[str, Any]) -> Optional[int]:
        try:
            now = datetime.now()
            q = (
                "INSERT INTO customers (name, phone, email, address, credit_limit, current_balance, created_at, updated_at) "  # noqa: E501
                "VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
            )
            params = (
                ...
                float(customer_data.get("credit_limit", 0)),
                float(customer_data.get("current_balance", 0)),
                now,
                now,
            )
    ...
    def get_lead(self, lead_id: int) -> Optional[Lead]:
        ...
        try:
            ...
            if row:
                # Handle created_at/updated_at that may be str or datetime
                def _parse_dt(val):
                    if val is None:
                        return None
