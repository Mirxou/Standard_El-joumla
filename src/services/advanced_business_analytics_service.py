...
class AdvancedBusinessAnalyticsService:
    ...
    def generate_business_insights(self, insight_types: Optional[List[str]] = None) -> List[BusinessInsight]:
        ...
        try:
            ...
            insights = []
            available_types = insight_types or ['performance', 'trend', 'anomaly', 'opportunity']
    ...
    def perform_customer_segmentation(self) -> List[CustomerSegment]:
        ...
        try:
            ...
            features = ['total_purchases', 'avg_order_value', 'purchase_frequency',
                       'days_since_last_purchase', 'total_spent', 'product_categories']
    ...
    def calculate_business_metrics(self, categories: Optional[List[str]] = None) -> List[BusinessMetric]:
        ...
        try:
            ...
            metrics = []
            available_categories = categories or ['financial', 'operational', 'customer', 'product']
        ...
        try:
            ...
            if sales_performance.get('change_percentage', 0) > 0.1:
                insight = BusinessInsight(
                    ...
                    title='أداء المبيعات',
                    description=f"المبيعات {'ارتفعت' if sales_performance['change_percentage'] > 0 else 'انخفضت'} بنسبة {abs(sales_performance['change_percentage'])*100:.1f}%",
                    data_points=sales_performance,
                    confidence_score=0.85,
                    impact_level='high',
                    recommended_actions=['مراجعة استراتيجية المبيعات', 'تحليل أسباب التغيير'],
                    generated_at=datetime.now()
                )
        ...
        try:
            ...
            if sales_trends.get('trend_strength', 0) > 0.1:
                ...
                insight = BusinessInsight(
                    ...
                    impact_level='medium',
                    recommended_actions=['توقع الطلب المستقبلي', 'تعديل استراتيجية المخزون'],
                    generated_at=datetime.now()
                )
                insights.append(insight)
    ...
    def _generate_anomaly_insights(self) -> List[BusinessInsight]:
        ...
        try:
            ...
            for anomaly in sales_anomalies:
                insight = BusinessInsight(
                    insight_id=f"INSIGHT_ANOMALY_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000,9999)}",
    ...
    def _generate_opportunity_insights(self) -> List[BusinessInsight]:
        ...
        try:
            ...
            for opportunity in growth_opportunities:
                insight = BusinessInsight(
                    insight_id=f"INSIGHT_OPPORTUNITY_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{random.randint(1000,9999)}",
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                for row in cursor.fetchall():
                    customer = dict(row)
                    customer['days_since_last_purchase'] = (datetime.now() - customer['last_purchase_date']).days if customer['last_purchase_date'] else 365
        ...
        try:
            ...
            if forecast:
                insight = PredictiveInsight(
                    ...
                    confidence_interval=(sum(forecast.confidence_intervals[0]), sum(forecast.confidence_intervals[-1])),
        ...
        try:
            ...
            if churn_prediction:
                insight = PredictiveInsight(
                    ...
                    time_horizon=time_horizon,
                    influencing_factors=['customer_satisfaction', 'competition', 'pricing'],
    ...
    def _generate_product_predictive_insights(self, time_horizon: str) -> List[PredictiveInsight]:
        ...
        try:
            ...
            for product in top_products[:3]:  # أفضل 3 منتجات
                insight = PredictiveInsight(
                    insight_id=f"PRED_INSIGHT_PRODUCT_{product['product_id']}_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
                    prediction_type='product_demand',
                    target_metric='product_sales',
                    predicted_value=product['predicted_sales'],
                    confidence_interval=(product['predicted_sales'] * 0.8, product['predicted_sales'] * 1.2),
                    time_horizon=time_horizon,
    ...
    def _analyze_sales_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if previous_period == 0:
                return {'change_percentage': 0, 'current_sales': current_period, 'previous_sales': previous_period}
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                """, (
                    ...
                    json.dumps(insight.data_points), insight.confidence_score, insight.impact_level,
                    json.dumps(insight.recommended_actions), insight.generated_at, insight.expires_at
                ))
                conn.commit()
        except Exception as e:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                """, (
                    ...
                    json.dumps(segment.value_metrics), segment.created_at, segment.last_updated
                ))
                conn.commit()
        except Exception as e:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    metric.metric_id, metric.metric_name, metric.category, metric.current_value,
                    metric.previous_value, metric.target_value, metric.trend, metric.calculation_period,
                    metric.last_updated
                ))
                conn.commit()
        except Exception as e:
        ...
        try:
            with self.db.get_connection() as conn:
                ...
                cursor.execute("""
                    ...
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    insight.insight_id, insight.prediction_type, insight.target_metric,
                    insight.predicted_value, json.dumps(insight.confidence_interval),
                    insight.time_horizon, json.dumps(insight.influencing_factors),
                    json.dumps(insight.risk_assessment), insight.generated_at
                ))
                conn.commit()
        except Exception as e:
                ...
                if metric.trend == 'declining' and metric.target_value:
                    if metric.current_value < metric.target_value * 0.9:
                        recommendations.append({
                            ...
                            'issue': f'القيمة الحالية ({metric.current_value:.2f}) أقل من الهدف ({metric.target_value:.2f})',
