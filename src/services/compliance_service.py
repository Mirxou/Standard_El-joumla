...
class ComplianceService:
    ...
    def create_compliance_rule(self, rule: ComplianceRule) -> Optional[int]:
        ...
        try:
            ...
            values = (
                rule.name, rule.rule_type, rule.description, rule.rule_config,
                1 if rule.is_active else 0, rule.severity,
                company_id, rule.created_by
            )
    ...
    def update_compliance_rule(self, rule: ComplianceRule) -> bool:
        ...
        try:
            ...
            params = [
                ...
                rule.rule_config, 1 if rule.is_active else 0,
    ...
    def _check_data_retention(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            table_name = config.get("table_name", "sales")
    ...
    def _save_compliance_check(self, check: ComplianceCheck) -> Optional[int]:
        ...
        try:
            ...
            values = (
                check.rule_id, check.check_date.isoformat() if check.check_date else None,
                check.status, check.result, check.notes,
                company_id, check.checked_by
            )
    ...
    def request_gdpr_erasure(self, customer_id: int, reason: str = "", requested_by: Optional[int] = None) -> Optional[int]:
    ...
    def get_gdpr_requests(self, status: Optional[str] = None, request_type: Optional[str] = None) -> List[Dict[str, Any]]:
