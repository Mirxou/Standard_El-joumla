...
class AuditLogService:
    ...
    def _detect_audit_schema(self) -> dict:
        ...
        Canonical fields: id, user_id, username, action, module, entity_type, entity_id, old_values, new_values, changes_summary, ip_address, user_agent, session_id, status, error_message, timestamp  # noqa: E501
        """
        res = {
            "table": None,
            "id": None,
    ...
    def log_logout(self, user_id: int, username: str, session_id: str, **kwargs) -> int:
    ...
    def get_user_activity(self, user_id: int, days: int = 30) -> List[Dict]:
        ...
        logs, _ = self.search_audit_logs(user_id=user_id, start_date=start_date, limit=1000)
        return logs

    def get_entity_history(self, entity_type: str, entity_id: int) -> List[Dict]:
        """الحصول على تاريخ كيان"""
        logs, _ = self.search_audit_logs(entity_type=entity_type, entity_id=entity_id, limit=1000)
        return logs

    def get_recent_activity(self, hours: int = 24, limit: int = 100) -> List[Dict]:
        """الحصول على النشاط الأخير"""
        start_date = datetime.now() - timedelta(hours=hours)
        logs, _ = self.search_audit_logs(start_date=start_date, limit=limit)
        return logs
    ...
    def get_user_statistics(self, user_id: int, days: int = 30) -> Dict:
        ...
        if tbl is None:
            return {
                ...
                "actions_by_module": {},
                "success_rate": 0,
                "most_active_day": {"date": None, "count": 0},
            }
        ...
        # Total actions
        cursor.execute(
            f"""SELECT COUNT(*) FROM {tbl} WHERE user_id = ? AND {ts} >= ?""",
            (user_id, start_date),
        )
        ...
        # Success rate
        cursor.execute(
            f"""SELECT SUM(CASE WHEN status = 'success' THEN 1 ELSE 0 END) * 1.0 / COUNT(*) * 100 FROM {tbl} WHERE user_id = ? AND {ts} >= ?""",  # noqa: E501
            (user_id, start_date),
        )
        ...
        # Most active day
        cursor.execute(
            f"""SELECT date({ts}) as day, COUNT(*) as count FROM {tbl} WHERE user_id = ? AND {ts} >= ? GROUP BY day ORDER BY count DESC LIMIT 1""",  # noqa: E501
            (user_id, start_date),
        )
        row = cursor.fetchone()
    ...
    def get_module_statistics(self, module: str, days: int = 30) -> Dict:
        ...
        # Total actions
        cursor.execute(
            f"""SELECT COUNT(*) FROM {tbl} WHERE {mod} = ? AND {ts} >= ?""",
            (module, start_date),
        )
        stats["total_actions"] = cursor.fetchone()[0]
    ...
    def get_active_sessions(self, user_id: int = None) -> List[Dict]:
        ...
        if self._table_exists("v_active_sessions"):
            ...
            if user_id:
                cursor.execute(
                    """
                    SELECT s.session_id, s.user_id, u.username, u.full_name, s.login_time, s.last_activity, s.ip_address, s.user_agent,  # noqa: E501
                           ROUND((JULIANDAY('now') - JULIANDAY(COALESCE(s.last_activity, s.login_time))) * 24 * 60, 2) AS idle_minutes  # noqa: E501
