        ...
        try:
            ...
            for dev in devices:
                if dev.bDeviceClass == 7:  # Printer class
                    try:
                        name = (
                            usb.util.get_string(dev, dev.iProduct) or f"Printer {dev.idVendor:04x}:{dev.idProduct:04x}"
                        )
                        printers.append(
                            {
                        ...
                        logging.getLogger(__name__).warning("Ignored exception in direct_print_service.py")
