...
class ReminderService:
    ...
    def schedule_payment_reminder(
        ...
        try:
            ...
            self.db.execute_query(
                """
                INSERT INTO reminders (sale_id, customer_id, reminder_type, subject, message, due_at, recipient_email, created_by)  # noqa: E501
                VALUES (?, ?, 'payment', ?, ?, ?, ?, ?)
                """,
                (
                    sale_id,
            ...
            logger.log(logging.ERROR, f"Failed to schedule payment reminder: {e}")
    ...
    def list_pending(self, limit: int = 100) -> List[Dict[str, Any]]:
        try:
            ...
            logger.log(logging.ERROR, f"Failed to list pending reminders: {e}")
            return []
    ...
    def send_due_reminders(self, attach_invoice_pdf: bool = True) -> Dict[str, Any]:
        ...
        for r in pending:
            try:
                ...
                data = {**invoice, "due_date": r.get("due_at")[:10]}
                ...
                if attach_invoice_pdf:
                    ...
                    if pdf_res.get("success") and pdf_res.get("pdf_path"):
                        ...
                        try:
                            ...
                            logging.getLogger(__name__).warning("Ignored exception in reminder_service.py")
                ...
                if send_res.get("success"):
                    self.db.execute_query(
                        "UPDATE reminders SET status='sent', attempts=attempts+1, last_attempt_at=CURRENT_TIMESTAMP WHERE id=?",  # noqa: E501
                        (r.get("id"),),
                    )
                    sent += 1
                else:
                ...
                failed += 1
                logger.log(logging.ERROR, f"Reminder send failed: {e}")
                try:
                    self.db.execute_query(
    ...
    def cancel_reminder(self, reminder_id: int) -> bool:
        try:
            ...
            logger.log(logging.ERROR, f"Failed to cancel reminder: {e}")
            return False
