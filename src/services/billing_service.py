...
class BillingService:
    ...
    def generate_invoice(self, sale_id: int, due_days: int = 0) -> Optional[int]:
        try:
            ...
            sale = self.db.fetch_one(
                "SELECT id, customer_id, total_amount FROM sales WHERE id = ?",
                (sale_id,),
            )
            if not sale:
                if self.logger:
    ...
    def record_payment(
        ...
        try:
            ...
            try:
                ...
                # Fallback to core payments schema (payment_type, entity_id, amount, payment_method, reference_number, payment_date, status, notes, user_id, created_at, updated_at)  # noqa: E501
                msg = str(e)
                ...
                if "no column" in msg or "no such column" in msg:
                    ...
                    q2 = """INSERT INTO payments (payment_type, entity_id, amount, payment_method, reference_number, payment_date, status, created_at, updated_at)  # noqa: E501
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
                    params2 = (
                        "customer_payment",
                        invoice_id,
    ...
    def create_recurring_invoice(self, subscription_id: int) -> Optional[int]:
        ...
        try:
            sub = self.db.fetch_one(
                "SELECT id, customer_id, amount, frequency_days FROM subscriptions WHERE id = ?",
                (subscription_id,),
            )
            if not sub:
                return None
