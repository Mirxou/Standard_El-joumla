...
class PermissionService:
    ...
    def check(self, user_id: Any, permission_code: str) -> bool:
        ...
        try:
            ...
            if not user or not user.role_id:
                return False
            perms = self.get_role_permissions(user.role_id)
            return any(p.code == permission_code for p in perms)
        except Exception:
            return False

    def grant(self, user_id: Any, permission_code: str) -> bool:
        """منح صلاحية لمستخدم"""
        return True

    def revoke(self, user_id: Any, permission_code: str) -> bool:
        """إلغاء صلاحية من مستخدم"""
        return True

    def get_permissions(self, user_id: Any) -> List[str]:
    ...
    def _init_default_permissions(self):
        ...
        for perm in default_permissions:
            existing = self.db.execute_query("SELECT id FROM permissions WHERE code = ?", [perm["code"]])
            if not existing:
    ...
    def create_permission(self, permission: Permission) -> int:
        ...
        return self.db.execute_update(
            ...
            [
                ...
                int(permission.is_system),
            ],
        )

    def get_permission(self, permission_id: int) -> Optional[Permission]:
        """الحصول على صلاحية بالمعرف"""
        result = self.db.execute_query("SELECT * FROM permissions WHERE id = ?", [permission_id])
        if not result:
            return None
    ...
    def list_permissions(self, resource_type: Optional[ResourceType] = None) -> List[Permission]:
        ...
        if resource_type:
            ...
            result = self.db.execute_query("SELECT * FROM permissions ORDER BY resource_type, name")
    ...
    def _init_default_roles(self):
        ...
        admin_role = self.db.execute_query("SELECT id FROM roles WHERE code = 'ADMIN'")
        if not admin_role:
        ...
        # دور المحاسب
        accountant_role = self.db.execute_query("SELECT id FROM roles WHERE code = 'ACCOUNTANT'")
        if not accountant_role:
            ...
            for code in perm_codes:
                perm = self.db.execute_query("SELECT id FROM permissions WHERE code = ?", [code])
                if perm:
                    self.db.execute_update(
                        "INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)",
                        [role_id, perm[0]["id"]],
        ...
        # دور موظف المبيعات
        sales_role = self.db.execute_query("SELECT id FROM roles WHERE code = 'SALES_STAFF'")
        if not sales_role:
            role_id = self.db.execute_update(
                """INSERT INTO roles (name, code, description, is_system, is_active)
                   VALUES (?, ?, ?, ?, ?)""",
    ...
    def get_role(self, role_id: int) -> Optional[Role]:
        ...
        result = self.db.execute_query("SELECT * FROM roles WHERE id = ?", [role_id])
        if not result:
            return None
        ...
        role = Role(
            ...
            created_at=(
                ...
                if isinstance(role_data.get("created_at"), datetime)
                else (datetime.fromisoformat(role_data["created_at"]) if role_data.get("created_at") else None)
            ),
            updated_at=(
                role_data["updated_at"]
                if isinstance(role_data.get("updated_at"), datetime)
                else (datetime.fromisoformat(role_data["updated_at"]) if role_data.get("updated_at") else None)
            ),
        )
    ...
    def update_role_permissions(self, role_id: int, permission_ids: List[int]) -> bool:
        ...
        self.db.execute_update("DELETE FROM role_permissions WHERE role_id = ?", [role_id])
    ...
    def list_roles(self, active_only: bool = False) -> List[Role]:
        """قائمة جميع الأدوار"""
        if active_only:
            result = self.db.execute_query("SELECT * FROM roles WHERE is_active = 1 ORDER BY name")
        else:
            result = self.db.execute_query("SELECT * FROM roles ORDER BY name")
    ...
    def list_users(self, active_only: bool = False) -> List[User]:
        ...
        if active_only:
            ...
            result = self.db.execute_query("SELECT * FROM users ORDER BY full_name")
    ...
    def count_users_in_role(self, role_id: int) -> int:
        ...
        result = self.db.execute_query("SELECT COUNT(*) as count FROM users WHERE role_id = ?", [role_id])
