...
class InventoryCountService:
    ...
    def reject_adjustment(self, adjustment_id: int, user_id: int, user_name: str, reason: str) -> bool:
        """رفض تسوية"""
        adjustment = self.get_adjustment_by_id(adjustment_id)
        if not adjustment:
            return False
    ...
    def create_adjustments_from_count(self, count_id: int, user_id: int, user_name: str) -> int:
    ...
    def get_count_summary(self, from_date: Optional[date] = None, to_date: Optional[date] = None) -> Dict[str, Any]:
        """ملخص الجرود"""
        query = """
            SELECT
    ...
    def _row_to_count_item(self, row: Dict) -> CountItem:
        ...
        return CountItem(
            ...
            counted_quantity=(
                Decimal(str(row["counted_quantity"])) if row.get("counted_quantity") is not None else None
            ),
