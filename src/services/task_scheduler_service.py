...
class TaskScheduler:
    def __init__(self, db_manager, reminder_service: Optional[ReminderService] = None, interval_seconds: int = 60):
            ...
            for job in self._jobs:
                if current_time - job['last_run'] > job['interval']:
                    try:
                        ...
                        logger.error(f"Scheduler job {job['func'].__name__} failed: {e}")
