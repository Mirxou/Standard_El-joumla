...
class CacheService:
    ...
    def __init__(self, logger=None):
        ...
        self.caches = {
            ...
            "queries": (
                RedisCache(redis_url, prefix="queries:")
                if (use_redis and redis_available and RedisCache)
                else LRUCache(max_size=200, default_ttl=60)
            ),
            "general": (
                RedisCache(redis_url, prefix="general:")
                if (use_redis and redis_available and RedisCache)
                else LRUCache(max_size=500, default_ttl=300)
            ),
        }
        ...
        def cleanup_loop():
            ...
            while True:
                ...
                if service is None:
                    break
                try:
                    for cache in service.caches.values():
                        cache.cleanup_expired()
                except Exception as e:
                    logging.getLogger(__name__).warning(f"Cache cleanup error: {e}")

        # Skip cleanup thread in headless offscreen test environment
