...
class VendorService:
    ...
    def create_purchase_order(self, vendor_id: int, items: List[Dict[str, Any]], meta: Dict[str, Any] = None) -> Optional[int]:
        try:
            ...
            q = ('INSERT INTO purchase_orders (po_number, supplier_id, status, total_amount, order_date, created_at, updated_at, notes) '
                 'VALUES (?, ?, ?, ?, ?, ?, ?, ?)')
            params = (po_number, vendor_id, 'pending', total, now, now, now, str(meta or {}))
            ...
            for it in items:
                ...
                q_item = ('INSERT INTO purchase_order_items (purchase_order_id, product_id, quantity_ordered, unit_price, subtotal, created_at) '
                          'VALUES (?, ?, ?, ?, ?, ?)')
    ...
    def receive_purchase(self, purchase_id: int, received_items: List[Dict[str, Any]]) -> bool:
        try:
            ...
            self.db.execute_query('UPDATE purchase_orders SET status = ?, delivery_date = ?, updated_at = ? WHERE id = ?', 
            ...
            for it in received_items:
                ...
                qty = float(it.get('quantity', 1))
                self.db.execute_query('UPDATE purchase_order_items SET quantity_received = ?, actual_delivery_date = ? WHERE purchase_order_id = ? AND product_id = ?',
                ...
                try:
                    ...
                    movement = StockMovement(product_id=pid, movement_type='شراء', quantity=int(qty), reference_id=purchase_id, reference_type='purchase_order')
    ...
    def generate_demand_plan(self, ai_service, days_ahead: int = 30) -> List[Dict[str, Any]]:
        ...
        plan = []
        try:
            products = self.db.fetch_all('SELECT id, name, current_stock, min_stock FROM products') # min_stock instead of reorder_level
            for p in products:
