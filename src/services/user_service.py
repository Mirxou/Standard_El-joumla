...
from ..models.user import Permission, User, UserManager, UserRole
...
class UserService:
    ...
    def _load_security_settings(self):
        """تحميل إعدادات الأمان من قاعدة البيانات"""
        try:
            settings = self.db.execute_query("SELECT setting_key, setting_value FROM security_settings")
        ...
        try:
            ...
            if self._is_user_locked(username):
                self._log_login_attempt(
                    ...
                    ip_address,
                    False,
                    "المستخدم مقفل بسبب محاولات فاشلة متكررة",
                )
                return (
                    False,
                    None,
                    "المستخدم مقفل مؤقتاً بسبب محاولات تسجيل دخول فاشلة متكررة",
                )
    ...
    def logout_user(self, session_id: str) -> bool:
        ...
        try:
            session = self.active_sessions.get(session_id)
            if session:
                self.db_logger.log_operation("logout", "system", session.user_id)
    ...
    def _validate_password_strength(self, password: str) -> Tuple[bool, Optional[str]]:
        ...
        if len(password) < self.security_settings.password_min_length:
            return (
                False,
                f"كلمة المرور يجب أن تكون {self.security_settings.password_min_length} أحرف على الأقل",
            )
        ...
        if self.security_settings.password_require_symbols and not any(
            ...
            for setting_name, setting_value in settings.items():
                ...
                if hasattr(self, "db") and self.db:
                    self.db.execute_update(
                        """
                        INSERT OR REPLACE INTO security_settings (setting_key, setting_value, updated_at)
                        VALUES (?, ?, ?)
                    """,
