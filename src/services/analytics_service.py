        ...
        try:
            ...
            if not rows:
                return {
                    "success": True,
                    "data": [],
                    "trends": {"total_sales": 0.0, "avg_daily": 0.0},
                }
            ...
            if PANDAS_AVAILABLE and len(data) > 1:
                ...
                # حساب معدل النمو
                growth_rate = (
                    ((df["total_amount"].iloc[-1] - df["total_amount"].iloc[0]) / df["total_amount"].iloc[0] * 100)
                    if df["total_amount"].iloc[0] > 0
                    else 0
                )
            ...
            try:
                ...
                logging.getLogger(__name__).warning("Ignored exception in analytics_service.py")
            return {"error": str(e)}
