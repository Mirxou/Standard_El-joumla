...
class ProductService:
    ...
    def create_product(self, product: Product) -> Optional[int]:
        ...
        try:
            ...
            col_map = {
                ...
                "specifications": (
                    json.dumps(product.specifications, ensure_ascii=False) if product.specifications else None
                ),
                "unit": product.unit,
                ...
                "selling_price": (
                    float(product.base_price) if getattr(product, "base_price", None) is not None else None
                ),
            ...
            self.db_manager.execute_query(query, tuple(insert_vals))
            try:
    ...
    def calculate_product_price(
        self, product_id: int, quantity: int = 1, customer_type: Optional[str] = None
    ) -> Optional[Decimal]:
    ...
    def _row_to_product(self, row, col_names=None) -> Product:
        ...
        return Product(
            ...
            unit=get_value("unit"),
            cost_price=(
                Decimal(str(get_value("cost_price", 0))) if get_value("cost_price") is not None else Decimal("0")
            ),
            ...
            average_rating=(
                Decimal(str(get_value("average_rating", 0)))
                if get_value("average_rating") is not None
                else Decimal("0")
            ),
        )
