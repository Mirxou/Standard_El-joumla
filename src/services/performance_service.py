...
class PerformanceService:
    ...
    def _analyze_tables(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "tables_analyzed": analyzed}
        except Exception as e:
    ...
    def _reindex_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "message": "تمت إعادة بناء جميع الفهارس"}
        except Exception as e:
            return {"success": False, "error": str(e)}
    ...
    def repair_database(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if integrity.get("is_healthy"):
                return {"success": True, "message": "قاعدة البيانات لا تحتاج إلى إصلاح"}
    ...
    def cleanup_data(self, days: int = 90) -> Dict[str, Any]:
        ...
        try:
            ...
            audit_deleted = self.db.execute_update("DELETE FROM audit_logs WHERE created_at < ?", [cutoff_date])
            deleted_count += audit_deleted

            # حذف سجلات تسجيل الدخول القديمة
            login_deleted = self.db.execute_update("DELETE FROM login_history WHERE created_at < ?", [cutoff_date])
            deleted_count += login_deleted

            # حذف سجلات البحث القديمة
            search_deleted = self.db.execute_update("DELETE FROM search_history WHERE created_at < ?", [cutoff_date])
    ...
    def analyze_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            table_sizes = {}
            for table in tables:
                size_result = self.db.execute_query(f"SELECT COUNT(*) as count FROM {table}")
            ...
            analysis["largest_tables"] = [{"table": table, "records": count} for table, count in largest_tables]
    ...
    def get_table_statistics(self, table_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # معلومات الجدول
            table_info = self.db.execute_query(f"PRAGMA table_info({table_name})")
            stats["columns"] = [
                {
                    "name": col["name"],
                    "type": col["type"],
            ...
            # الفهارس
            index_list = self.db.execute_query(f"PRAGMA index_list({table_name})")
            stats["indexes"] = [
                {
                    "name": idx["name"],
            ...
            return {"success": True, "table": table_name, "statistics": stats}
    ...
    def _get_all_tables(self) -> List[str]:
        ...
        result = self.db.execute_query("SELECT name FROM sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'")
        return [row["name"] for row in result]

    def _get_all_indexes(self) -> List[str]:
        """الحصول على قائمة جميع الفهارس"""
        result = self.db.execute_query("SELECT name FROM sqlite_master WHERE type='index' AND name NOT LIKE 'sqlite_%'")
        return [row["name"] for row in result]
    ...
    def apply_performance_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": True, "settings_applied": settings_applied}
    ...
    def get_system_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"success": False, "error": "مكتبة psutil غير متوفرة"}
        except Exception as e:
            return {"success": False, "error": str(e)}
        ...
        def monitor_loop():
            ...
            while self._monitoring:
                try:
                    ...
                    logging.getLogger(__name__).warning("Ignored exception in performance_service.py")
    ...
    def get_slow_queries_from_db(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            result = self.db.execute_query("SELECT * FROM slow_queries ORDER BY executed_at DESC LIMIT ?", (limit,))
