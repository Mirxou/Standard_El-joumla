...
class InventoryService:
    ...
    def record_stock_movement(self, movement: StockMovement) -> Optional[int]:
        ...
        try:
            ...
            if result and hasattr(result, "lastrowid"):
                ...
                if self.logger:
                    self.logger.info(
                        f"تم تسجيل حركة مخزون - النوع: {movement.movement_type}, المنتج: {movement.product_id}"
                    )
    ...
    def release_reserved_stock(self, product_id: int, quantity: int, reference_id: Optional[int] = None) -> bool:
        """تحرير مخزون محجوز"""
        try:
            movement = StockMovement(
                product_id=product_id,
    ...
    def create_inventory_count(
        self, count_date: datetime, counted_by: str, notes: Optional[str] = None
    ) -> Optional[int]:
        """إنشاء عملية جرد"""
        try:
            query = """
            ...
            if result and hasattr(result, "lastrowid"):
                if self.logger:
                    self.logger.info("تم إنشاء عملية جرد جديدة")
    ...
    def get_inventory_turnover_rate(self, product_id: int, days: int = 30) -> Decimal:
        ...
        try:
            ...
            result = self.db_manager.fetch_one(query, (product_id, MovementType.SALE.value, start_date))
    ...
    def get_inventory_report(self) -> InventoryReport:
        ...
        try:
            ...
            ("reserved_stock" if "reserved_stock" in available_cols else "0 as reserved_stock")
            "max_stock" if "max_stock" in available_cols else "999999 as max_stock"
            base_price = (  # noqa: F841
                "base_price"
                if "base_price" in available_cols
                else ("selling_price" if "selling_price" in available_cols else "0")
            )
