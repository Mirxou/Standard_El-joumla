...
class DynamicPricingEngine:
    ...
    def adjust_price(self, current_price: Decimal, product_id: int,
                    customer: Customer, quantity: Decimal) -> Decimal:
        """
    ...
    def _calculate_signals_adjustment(self, base_price: Decimal, product_id: int,
                                    customer: Customer, quantity: Decimal) -> Decimal:
        ...
        try:
            ...
            if stock_info and stock_info[0]:
                ...
                # Low stock warning
                    ...
                    confidence = min(1.0, float((min_stock * Decimal('1.5') - current_stock) / (min_stock * Decimal('1.5'))))
                    signals.append(PricingSignal(
                        signal_type='inventory',
                        product_id=product_id,
                ...
                # Excess stock
                    ...
                    confidence = min(1.0, float((current_stock - min_stock * Decimal('3')) / (min_stock * Decimal('3'))))
                    signals.append(PricingSignal(
                        signal_type='inventory',
                        product_id=product_id,
    ...
    def _rule_applies(self, rule: DynamicPricingRule, product_id: int, customer: Customer, quantity: Decimal) -> bool:
    ...
    def get_pricing_insights(self, product_id: int, customer: Customer = None, quantity: Decimal = Decimal('1')) -> Dict[str, Any]:
        """الحصول على رؤى التسعير المتقدمة"""
        try:
            base_price = self.db_manager.fetch_one(
                "SELECT retail_price FROM products WHERE id = ?",
                (product_id,)
            )
            ...
            total_adjustment = inventory_adj + demand_adj + loyalty_adj + quantity_adj + time_adj + signals_adj + rules_adj
    ...
    def _generate_recommendations(self, total_adjustment: Decimal, signals: List[PricingSignal]) -> List[str]:
        ...
        if 'inventory' in signal_types and any(s.adjustment_percentage > 0 for s in signals if s.signal_type == 'inventory'):
        ...
        try:
            ...
            recent_sales = Decimal(str(sales_data[0]))
            avg_daily_sales = recent_sales / Decimal('30')

            if avg_daily_sales >= self.DEMAND_HIGH_THRESHOLD:
            ...
            self.logger.error(f"خطأ في حساب تعديل الطلب: {str(e)}")
            return Decimal('0.00')
    ...
    def _get_minimum_price(self, product_id: int) -> Decimal:
        ...
        try:
            cost_info = self.db_manager.fetch_one(
    ...
    def get_price_recommendation(self, product_id: int) -> Dict[str, Any]:
        ...
        try:
            ...
            mock_customer = Customer(id=1, name="Test Customer")
