...
class AuditService:
    ...
    def get_resource_history(self, resource_type: str, resource_id: int, limit: int = 50) -> List[AuditLog]:
        """سجل تاريخ مورد معين"""
    ...
    def cleanup_old_logs(self, days: int = 90) -> int:
        ...
        audit_deleted = self.db.execute_update("DELETE FROM audit_logs WHERE created_at < ?", [cutoff_date.isoformat()])

        # حذف سجلات تسجيل الدخول القديمة
        login_deleted = self.db.execute_update(
            "DELETE FROM login_history WHERE created_at < ?", [cutoff_date.isoformat()]
        )
